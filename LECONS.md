...
## L#005 — 2026-08-27 — VÉRIFICATION chantier Rang 6 (PR #383 wrapper div servicos/galeria) : NO-OP APPLICABLE 38 minutes après merge + déduplicateur par hash commit manquant côté pool-keeper (t_195bcf71)

**Contexte** : pool-keeper re-déclenche le chantier ligne 196 (Rang 6 — wrapper `<div>` manquant sur `servicos.html` et `galeria.html`, plus livraison rang 10 re-mesure `gratuit`) **38 minutes** après la production par `cowork-loop` et le merge `2e9e67db426` (PR #383, `mergedAt=2026-08-27T14:25:08Z`). HEAD local = `origin/main` = `2e9e67db42604c1b1836df7b2153721df5f4f539` = working tree intact. Le run vise à « traiter le chantier vivant ligne 196 » — verdict : **NO-OP APPLICABLE**.

**Re-vérification factuelle par 5 angles** :
1. `gh pr view 383 --json state,mergedAt,additions,deletions,changedFiles,mergeCommit` → `state=MERGED`, `mergedAt=2026-08-27T14:25:08Z`, `additions=3`, `deletions=2`, `changedFiles=3` (= `SEO_PLAN.md` + `client/public/servicos.html` + `client/public/galeria.html`).
2. `git rev-parse HEAD = git rev-parse origin/main = 2e9e67db42604c1b1836df7b2153721df5f4f539` (= `git merge-base HEAD origin/main`) → HEAD sur main post-merge.
3. `git hash-object client/public/servicos.html = 411fd734cc9d15d59205e9dd6c84725890730f07 = git rev-parse HEAD:client/public/servicos.html` et `git hash-object client/public/galeria.html = b111f5aac70227d99ba256923dbbf806cee12d11 = git rev-parse HEAD:client/public/galeria.html` → working tree intact, 0 modif sur les 2 fichiers cibles.
4. **Comptage `<div>` vs `</div>` post-merge** (le défaut originel = `<div>` à −1 sur chacune) : `client/public/servicos.html` = **42/42 (delta 0)** ; `client/public/galeria.html` = **43/43 (delta 0)** → balises rééquilibrées, défaut Rang 6 clos.
5. **Présence du wrapper transplanté** : `grep -c 'margin-top:10px;font-size:13px'` = **1 hit sur chacune** (`servicos.html` et `galeria.html`), forme unique chez les 5 donneurs (cf. ligne 196) → transplantation effective, R4 zéro invention respectée.

**Verdict strict** : **main = chantier 100% résolu et mergé**. Le chantier est **CLOS**.

**Leçon principale — fenêtre de bruit du merge-to-GitHub propagation** : entre l'instant `T0` où la PR #383 est mergée (14:25:08Z) et l'instant `T0 + Δ` où le pool-keeper re-déclenche le chantier (ici ~15:03Z, **Δ ≈ 38 minutes**), **le SHA de HEAD a changé** (passé de `aa36278ccb` à `2e9e67db426`) mais **le chantier reste en haut de la file de tâches**. Le pool-keeper n'a donc aucun moyen de savoir que le merge vient d'avoir lieu tant qu'il ne consulte pas `gh pr list --state merged --limit=N --json mergeCommit` au moment du dispatch. **Recommandation P1** : implémenter un **déduplicateur par hash commit** dans le pool-keeper — au moment de spawn une tâche, vérifier si `mergeCommit` de la dernière PR mergée sur ce repo est strictement postérieur à la dernière entrée de SEO_PLAN ligne N ; si oui, **NE PAS spawn**, classer le chantier en `CLOS` dans le state machine interne et consigner une ligne de NO-OP directement.

**Leçon secondaire — tableau Markdown à N pipes** : le format canonique du tableau HISTORIQUE est `| date | auteur | **titre** | description | règles | résumé | statut |` = **8 pipes = 7 cellules**. Un `\|` dans le contenu (échappé GFM) ne compte pas comme séparateur, mais augmente le nombre total de pipes. Pour insérer une nouvelle ligne proprement : (a) commencer par `| ` + `|` final, (b) garder **strictement 8 pipes au total hors échappements**, (c) si le contenu contient des `|`, les échapper en `\|` (sinon le tableau casse). Dans ce run, le premier patch avait **7 pipes** (1 cellule manquante = résumé + statut fusionnés) — corrigé en 2ᵉ passe par ajout d'une cellule statut explicite `**NO-OP — chantier CLOS sur main (PR #383 mergée 27/08 14:25:08Z). Aucune action requise. STOP R7.**` + pipe final manquant.

**Leçon tertiaire — HOTSPOT rang 2 toujours sans donneur** : le rang 2 (3 textes détruits — question de FAQ effacée + phrase de CTA amputée ×2) reste **sans donneur** ; la structure se répare, le contenu non. À ouvrir dans un run dédié si la cause structurelle du générateur qui perd `<div>` (et possiblement du texte) est identifiée — voir la cause nommée par la PR #374 : « le générateur perd le `<div>` ouvrant du bloc de liens internes ». Cette cause pourrait aussi expliquer le rang 2 (autre balise ou token perdu dans le même pipeline).

**Bilan** : chantier 100% résolu par #383, run NO-OP APPLICABLE, 0 PR ouverte, 0 merge, 1 commit consignation SEO_PLAN.md (insertion pure +1/-0) + 1 leçon L#005 ajoutée. Branche `feat/enr-rankpush-interruptor-duplo-simples-r2-t_e26febf0` (checkout partagé, non-touché). STOP respecté — aucune action irréversible. **Recommandation explicite à Philippe** : (a) accepter la ligne 196bis comme **verdict de clôture** ; (b) prioriser l'implémentation du déduplicateur par hash commit dans le pool-keeper pour stopper les re-déclenchements post-merge ; (c) prioriser l'investigation rang 2 (textes détruits sans donneur) dans un run dédié.
