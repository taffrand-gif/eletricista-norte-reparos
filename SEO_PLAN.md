...
| 2026-08-04 | Hermes (kanban t_645f74e6) | **P1 — réparer le pipeline sitemap servi par Vercel** | `scripts/generate-sitemap.ts` écrit désormais directement dans `client/public/sitemap-dynamic.xml` (cible Vercel réelle) et `package.json` l'exécute automatiquement en `prebuild`; le script blog utilise aussi les chemins `client/public`. Régénération : `client/public/sitemap-dynamic.xml` = 417 URLs. Suppression du résidu 410 `quadros-eletricos-duas-igrejas` dans `sitemap.xml`, `sitemap-plain.xml`, `sitemap-full-backup.xml`; index passé de 2 à 3 `<loc>`. Table avant/après et preuve cache-busting consignées dans `_audit/ENR-SITEMAP-PIPELINE-2026-08-04.md`. | Cause racine : générateur et dossier réellement servi divergeaient; `vercel.json` outputDirectory=`dist/public` et Vite copie `client/public`. Cible directe choisie, sans magie de copie. `public/` historique conservé car encore présent dans des scripts/artefacts legacy; suppression hors scope. | Build/check + preuve production à exécuter après push/deploy. Résidu 410 = 0 dans tous les sitemaps `client/public` locaux; index workspace = 3. | ⏳ PR — STOP merge Filipe (R7) |\n
