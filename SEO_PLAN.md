...
| 2026-09-05 | Hermes | **GSC gap T3-INFO 'domotica'** (t_6abb0a06) | Renforcement page existante `client/public/blog/domotica-casa-inteligente-guia.html` (pos 16.4 / 40 impr / 1 clic en 28j, fenêtre 4..20 = presque top3). La page cible est la **meilleure dédiée existante** à la query « domotica » (vérification : 4 pages blog `como-escolher-domotica-casa.html`, `domotica-casa-inteligente-guia-completo.html`, `quanto-custa-instalar-domotica.html`, `smart-home-instalacao.html` — toutes variantes ; `domotica-casa-inteligente-guia.html` est la plus générique et la mieux indexée). La page couvrait déjà le sujet en H1 « Domótica Casa Inteligente: Guia Completo 2026 » (alignement direct verbatim Google « domotica » via normalisation accent PT-PT), mais le H1 / title ne portaient que la version accentuée — la query GSC est « domotica » sans accent. **Patch chirurgical R4 strict (zéro invention)** : (a) meta keywords étendu avec « domotica » verbatim (sans accent) en plus de « domótica » accentué + « instalação domotica » + « automação residencial » ; (b) FAQPage schema.org enrichi avec **3 Q/R verbatim** sur la query — Q1 « O que é domotica? », Q2 « Quanto custa instalar domotica numa casa? », Q3 « Como instalar domotica em casa passo a passo? » (FAQPage passe de 2 mainEntity à 5, **toutes JSON-LD parsées OK** par validator post-patch) ; (c) section visible `<section id="domotica">` ajoutée juste avant le grid « Artigos Relacionados » avec `<h2>Domotica — Resposta Direta</h2>` + 3 FAQ `<details>` correspondantes — verbatim « domotica » répété 6 fois dans la section (densité locale forte, signal snippet top3) ; (d) Article JSON-LD `dateModified` bump 2026-06-06 → **2026-09-05** (signal freshness Google) + Article.description recentré verbatim « Domotica casa inteligente: o que é, vantagens, custos e instalação » (avant « Domótica casa inteligente: o que é, vantagens, custos, instalação »). **Aucune invention prix/zone/délai** (R4 strict) — prix mentionnés dans la nouvelle section FAQ = plages marché génériques déjà documentées dans l'article (« DIY 200-600 EUR », « intermédio 800-2 500 EUR », « KNX a partir de 3 000 EUR »), main d'ouvre 70 €/h + déplacement Z1-Z6 15-65 EUR = renvoi à PRICING.md (source de vérité, jamais de forfait inventé). **Structure environnante intacte** : H1 préservé (« Domótica Casa Inteligente: Guia Completo 2026 »), titre préservé, og:title/og:description/twitter:title/twitter:description préservés, Related grid préservé (liens vers `domotica-casa-inteligente-guia-completo.html`, `casa-inteligente-guia-iniciante.html`, `instalacao-eletrica-casa-nova-guia.html`), CTA + sticky-cta + float-wa + bandeau RGPD préservés. **Fichier cible confirmé servir en prod** : `client/public/blog/domotica-casa-inteligente-guia.html` (25359 bytes, HTTP 200 0.30s, md5 prod = md5 on-disk original avant patch = b733a54e768b8e71171ed1e693244bd3 → preuve que la prod sert bien `client/public/`). **Patch strictement chirurgical** : 4 substitutions ciblées via `patch` tool (keywords meta, FAQPage JSON-LD extension, Article JSON-LD headline+description+dateModified, ajout section `id="domotica"` avant Artigos Relacionados), 0 occurrence de corruption collatérale sur les 25 359 → 30 694 octets (+5 335 octets = strictement la section FAQ + verbatim). Branche `fix/enr-rankpush-domotica-t_6abb0a06` depuis `origin/main`. PR draft à ouvrir via `gh pr create --draft --base main --head fix/enr-rankpush-domotica-t_6abb0a06`. Aucun merge sans GO (R7). Impact à mesurer à J+7 via gsc-trajectoire-cron.sh : cible pos < 6 (snippet top3 potentiel via FAQPage enrichi + verbatim section dédiée + dateModified frais). | Gap GSC fenêtre 28j (pos 16.4 = page 2 haute → top3 = levier verbatim FAQ + section dédiée `id="domotica"` + fresh dateModified + extension keywords sans accent) | +3 FAQ JSON-LD verbatim / +3 FAQ `<details>` visible / +1 H2 section dédiée / +1 mention keywords / +1 signal freshness | ⏸ PR draft à ouvrir, attente GO Philippe (R7) |
...
**Dernière MAJ : 2026-09-05 09h45 BST — **GSC gap T3-INFO 'domotica'** (t_6abb0a06) : renforcement page `client/public/blog/domotica-casa-inteligente-guia.html` (pos 16.4 / 40 impr / 1 clic 28j). Patch chirurgical R4 strict : meta keywords étendu avec « domotica » verbatim (sans accent), FAQPage schema.org enrichi avec 3 Q/R verbatim (5 mainEntity total), section visible `<section id="domotica">` avec 3 FAQ `<details>`, Article JSON-LD dateModified bump 2026-06-06 → 2026-09-05 + description recentrée. 0 invention prix/zone/délai (renvoi PRICING.md). PR draft à ouvrir, attente GO Philippe (R7). Mesure impact J+7 via gsc-trajectoire-cron.sh.
