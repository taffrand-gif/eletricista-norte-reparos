...
| X-DISJ0 | Page dédiée « disjuntor dispara sem nada ligado » — GSC gap T2 URGENT | HAUTE | A_FAIRE | draft (PR à ouvrir) | — | `disjuntor dispara sem nada ligado` · fenêtre GSC 28j : pos 6.4 / 27 impr / 2 clics (presque top3, **meilleure page actuelle = NONE** — la query n'avait aucune page dédiée). Page dédiée `client/public/blog/blog-disjuntor-dispara-sem-nada-ligado.html` créée (271 lignes) avec title/H1 verbatim, 11 sections H2 distinctes (6 causes réelles + diagnóstico + quando chamar + preço + FAQ), TOC 11 ancres résolues, 5 blocs JSON-LD valides (Article + BreadcrumbList + HowTo + FAQPage 9 Q + Service), 10 liens internes cluster. Maillage : +1 lien entrant `disjuntor-dispara-constantemente.html` → `blog-disjuntor-dispara-sem-nada-ligado` (ancre verbatim, section « Artigos Relacionados »). Sitemap : +1 entrée `sitemap-blog.xml` (lastmod 2026-09-02, priority 0.8). Aucune invention prix/zone/délai (alimentation `PRICING.md`). PR draft à ouvrir sur `origin/main` branche `fix/enr-rankpush-disjuntor-dispara-sem-nada-ligado-r3-t_1bc024be`. 0 merge sans GO (R7). Impact à mesurer J+7 via `gsc-trajectoire-cron.sh` : cible pos < 4 (snippet potentiel via FAQPage enrichi + verbatim H1 + section dédiée). |
<!-- CHANTIERS:END -->
