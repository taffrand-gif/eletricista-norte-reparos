...
| 2026-07-20 | Hermes (kanban t_18bb693b) | **GEO-PROTO Mirandela anti-doorway** | Enrichissement de `client/src/pages/cidades/Mirandela.tsx` : H1 answer-first, données owned TomTom/prix, 37 localités réconciliées, intention installation/diagnostic, Service + FAQPage + speakable + areaServed. Correction pré-PR des contradictions owned (ville ≠ concelho), retrait `getCityAddress()` pour R5 et téléphone E.164 explicite R-TEL. Leçon L#003 ajoutée. | Prouver sur une seule page existante qu'une différenciation GEO factuelle peut dépasser le template sans web ni invention R11. | Gate anti-doorway reproductible sur source TSX : Jaccard médian 21,7 % vs 5 villes (≤55 %) ; 5-grams uniques 82,1 % (≥70 %) ; answer-first 50 mots ; 6+ faits sourcés owned ; build vert. | ⏳ PR draft — prototype unique, zéro scale/merge |
