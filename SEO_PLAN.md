...
| 2026-08-11 | Hermes (kanban t_8d27ce31) | **VÉRIFICATION chantier vivant ligne 197 (PR #314 « SEO(ENR): renforce page 'como instalar fechadura eletrica' ») — run NO-OP, livrable déjà produit par t_c380c12d** | Le pool-keeper a re-déclenché une vérification du chantier ligne 197 (marqueur ⏸) **2h22 après** la production initiale (t_c380c12d 14:23 → t_8d27ce31 16:45). Constat par lecture du repo + `gh pr view 314` + `gh pr checks 314` : PR **#314 OPEN, `isDraft: true`, `state: OPEN`, `merged: null`**, baseRefName=main, headRefName=`fix/seo-enr-fechadura-eletrica-2026-08-11-v2`, **2 fichiers, +160/-6** (scope strict respecté), Vercel **PASS** (deployment `BA6oCEmU8mRnhtf2DC9HFXwh3TK6`, 0 erreur). Livrable conforme au cahier des charges du run : page renforcée (+623 mots visibles 886→1509, title/meta/FAQPage/HowTo/Article schemas alignés query exacte), maillage interne 7 pages, custos PRICING.md verbatim, 0 invention (R4), 0 merge (R7). **Périmètre run atteint = 100 %** (la consigne était « 1 PR draft ou 1 ligne SEO_PLAN mise à jour » — la PR draft existe déjà et l'entrée HISTORIQUE ligne 197 documente précisément le livrable). Aucune action supplémentaire légitime sans régression vérifiée du commit `cc2397b51e`. **LEÇON** : (1) Le pool-keeper doit consommer le verdict AVANT de re-déclencher — un chantier ligne 197 avec marqueur ⏸ et PR draft OPEN n'est pas un chantier à retraiter, c'est un chantier en attente GO R7 ; (2) Le cycle de vie d'un chantier SEO INFO doit être : `ready → claimed → ⏸ PR draft → (GO R7) → ✅ merged → archive`, pas `ready → claimed → ⏸ PR draft → ⏸ PR draft → ⏸ PR draft` ; (3) Cette ligne elle-même devient le **3e marqueur identique** pour le même commit `cc2397b51e` (cf. ligne 197 = run initial, ligne 197bis = cette vérification), donc le pool-keeper ne devrait plus re-déclencher tant que `cc2397b51e` reste HEAD de `fix/seo-enr-fechadura-eletrica-2026-08-11-v2` non-mergé — sinon il faudrait un déduplicateur par hash commit dans le pool-keeper (cf. leçon #447). **⚠️ RECTIFICATION 2026-08-17 (t_6038a3ed)** : l'affirmation « PR #314 OPEN, isDraft=true, mergedAt=null » ci-dessus était vraie au moment du run (2026-08-11 16:45) mais est désormais **factuellement fausse** — `gh pr view 314 --json state,isDraft,mergedAt,mergeCommit` confirme `state=MERGED, isDraft=false, mergedAt=2026-08-17T15:47:14Z, mergeCommit.oid=a9a7c0cebd` ; le commit `a9a7c0ceb` est HEAD de `origin/main` (succède à `d7676886eb merge rebase #314`). **GO R7 reçu** : Philippe a fusionné PR #314 le 17/08 15:47 UTC. Le chantier ligne 197 (renforce `como instalar fechadura eletrica`) est **fermé ✅** et **archivé**. **LEÇON FINALE** : (1) Le déduplicateur par hash commit n'a effectivement **jamais été implémenté** dans le pool-keeper entre 11/08 et 17/08 — preuve : 5 runs de vérification NO-OP identiques (t_c380c12d init 11/08, t_8d27ce31 11/08 16:45, t_5f4a1895 12/08, t_4e6c28b3 12/08 00:08, t_6038a3ed 17/08 20:25), tous sur le même commit `cc2397b51e` / `a9a7c0ceb`, sans qu'aucun ne détecte le merge effectif (escalade P1 Philippe — voir PROTOCOLE-AGENTS §5.2 « STOP auto-merge sur main » confirmé respecté) ; (2) Tant que la consigne « vérifier chantier ⏸ + PR draft OPEN » reste littérale, le pool-keeper continuera à re-déclencher à l'infini sur les chantiers en attente GO — **fix structurel** : ajouter un check `gh pr view <N> --jq .mergedAt` AVANT de re-déclencher une « vérification chantier vivant », et ne pas re-déclencher si `mergedAt != null` ; (3) Les 5 entrées HISTORIQUE NO-OP sur PR #314 constituent un **artefact documentaire** (preuve de la chaîne de vigilance), pas un défaut — mais elles devraient être consolidées en **1 seule entrée RÉSOLUTION** (la présente) pour ne pas saturer la table ; (4) Ligne 206 (t_4e6c28b3) contient la même affirmation obsolète « PR #314 OPEN, isDraft=true » — non corrigée par ce run car hors périmètre strict du brief (ligne 204 uniquement), à traiter dans un run séparé si pertinent. **Périmètre run atteint = 100 %** : la présente rectification consomme le verdict et ferme définitivement le chantier. | R4 (zéro invention), R7 (PR draft, attente GO merge Filipe), R8 (témoin : +160/-6 sur 2 fichiers = scope strict), R11 (recompte live SHA `7401b15703` = HEAD `origin/main` contenant `a9a7c0ceb` post-merge) | ✅ **RÉSOLU 2026-08-17** — PR #314 MERGED 15:47:14Z (commit `a9a7c0ceb`), chantier ligne 197 archivé |
...
| 2026-08-04 | Hermes (kanban t_14f223d6) | **Consolidation GSC « instalar videoporteiro » (pos 9.6, 25 impr / 1 clic 28j)** | Les données GSC page×query désignent `/blog/como-instalar-videoporteiro-completo` comme pilier gagnant (63 impressions, dont 25 à pos 9.0 sur la query) contre 3 impressions pour `/blog/como-instalar-videoporteiro`. Canonical de la page perdante pointé vers le gagnant afin d’éviter la cannibalisation; contenu source conservé. | Aucun prix, délai, zone ou claim ajouté. Modification limitée à 1 canonical; contrôle post-déploiement requis: gagnant 200 avec contenu réel et perdant portant le canonical attendu. | ⏳ PR draft — STOP merge/déploiement Filipe (R7); mesure J+7 via gsc-trajectoire-cron.sh |
