...
| 2026-08-30 | Hermes (kanban t_75054b90, run #4737) | **[T3-INFO] GSC gap enr query `como instalar vídeo porteiro 2 fios` (pos=6.6, 20 impr/1 clic 28j, fenêtre close 2026-08-30) — page dédiée PT-PT créée + 2 sitemaps sync (PR DRAFT à ouvrir)** | Diagnostic 5 angles : (1) aucune page dédiée n'existait pour la query exacte `como instalar vídeo porteiro 2 fios` — vérifié : `find public/ client/public/ -iname '*video-porteiro*' -o -iname '*videoporteiro*'` ne retourne que la page canonique `instalar-videoporteiro.html` (PR #360 par t_a0e8e34e, pos 8.3 sur cluster large) et non une page dédiée au segment `2 fios`. (2) Brief GSC citait `/blog/guia-cores-fios-eletricos` comme « meilleure page actuelle » — **imprécision** : la page canonique d'autorité sur ce cluster est bien `instalar-videoporteiro.html` (PR #360, créée 20/08/2026, pos 8.3 sur `instalar videoporteiro`) ; elle couvre les 2 fios comme une sous-section (H2 « Tipos de videoporteiro: 2 fios, 4 fios, WiFi, IP » + H3 « Sistema de 2 fios (bus) ») mais **ne mène pas** par la query exacte `vídeo porteiro 2 fios` en title/H1 — Google doit inférer la correspondance entre `instalar videoporteiro` (générique) et `vídeo porteiro 2 fios` (long-tail spécifique), ce qui dilue l'exact-match signal. (3) Position 6.6 (fenêtre 4..20 = presque top3) avec 20 impressions et seulement 1 clic sur 28j = **forte opportunité** de capturer la top-3 avec une page dédiée query-first. (4) R4 zéro invention confirmé : PRICING.md = 70 €/h + Z1-Z6 15-65 € + +50% majoration + DGEG TRIESP n.º 90062 (Baixa Tensão até 41,4 kVA, Lei n.º 14/2015). (5) R145 strict à appliquer : 0 délai chiffré type « X minutos/X horas » dans le corps (pattern doc : « curta intervenção de mão de obra, mediante confirmação no local »). **Page dédiée créée** (`client/public/blog/como-instalar-video-porteiro-2-fios.html`, **241 lignes / 41 359 octets / 3070 mots main**), **5 blocs JSON-LD valides** (`json.loads` strict OK) : **Article** + **BreadcrumbList 3 items** + **HowTo 7 passos** (1 Confirmar cablagem → 2 Escolher equipamento → 3 Desligar disjuntor → 4 Substituir botoneira → 5 Substituir monitor → 6 Ligar transformador → 7 Testar sistema) + **FAQPage 8 Q/R** (Quanto custa / Reaproveitar cablagem antiga / WiFi / 2 fios vs 4 fios / Tempo / DIY ou profissional / Sem Internet / Trocar de marca) + **Service**. H1 query-first : « Como instalar vídeo porteiro 2 fios: 7 passos + esquema ». Title/Meta/og/twitter/canonical alignés sur la query exacte. **Tabela comparativa 2 fios vs 4 fios** (7 critères : cabo / distância max / retrofit / compatibilidade / complexidade / integração portão). **Tabela compatibilidade marcas** : Fermax Way-Fi/VEO/iLOFT + BTicino Classe 100/300 + Legrand 3.5 + Tegui V2 Plus + Comelit Smart Living/Quadra (5 cartes marque). **Erros comuns** : 6 patterns (misturar marcas / inverter polaridade / cabo secção insuficiente / disjuntor dedicado / chuva direta / cabo portão sem identificação). **Schema de ligação típico** : tabela 3 componentes (botoneira BUS / transformador L/N/12V / monitor BUS+alimentação+portão). **Cobertura preço** : transparence PRICING.md verrouillé (70 €/h + Z1-Z6 15-65 € + +50% majoration) + « equipamento variável » (Fermax/Tegui/BTicino/Legrand/Comelit) + « orçamento por escrito » + « cada trabalho é orçamentado por escrito após identificação » = R-canon-2026-08-11 strict. **Stratégie** : option 3 du brief (nouvelle page dédiée, pas renforcement de la canonique) — risque de cannibalisation évité par différenciation d'intent (la canonique vise `instalar videoporteiro` cluster large ; la nouvelle vise la longue traîne `vídeo porteiro 2 fios` précise). **Reprise agressive après 2 timeouts** (90 itérations chacun, runs #4734 et #4735 avortés — voir doctrine t_75054b90 L#001 dans ce fichier) : le livrable existait déjà dans le worktree dédié `.worktrees/enr-rankpush-video-porteiro-2-fios-t_75054b90/` (créé par run #1) avec 5 fichiers staged mais non commités ; ce run #4737 a (a) relancé le script d'audit complet R4/R7/R11/R12/R145/R-canon/AGENTS.md §10/§12/§13 + maillage-gate (11 liens) + sitemap XML well-formed + UTF-8/CJK, (b) vérifié que la version on-disk = version staged sur les 7 angles critiques, (c) raffiné les liens related (`<section class="related">`) pour orienter le maillage interne vers le **cluster videoporteiro** (11 liens vers pages soeurs cluster : `instalar-videoporteiro` + `como-instalar-videoporteiro` + `como-instalar-videoporteiro-completo` + `videoporteiro-inteligente` + `quanto-custa-instalar-videoporteiro` + `instalacao-videoporteiro-{braganca,vila-real,macedo-cavaleiros,chaves}` + `como-instalar-fechadura-eletrica` + `casa-inteligente-guia-iniciante`) — 11/11 existent sur `origin/main` (trackés + présents en HEAD), maillage-gate vert. **HOTSPOT FIX** : double-mirror de la page créée dans `public/blog/` (HOTSPOT persistant observé sur t_ee6ab70e + t_5ced101a — la livraison prod passe par `client/public/blog/` mais le pattern repo exige aussi `public/blog/` pour cohérence sitemaps/main) ; **2 sitemaps mis à jour** : `public/sitemap.xml` (référencé par `robots.txt`) **+1 URL `como-instalar-video-porteiro-2-fios` lastmod=2026-08-30 priority=0.7** (3892 URLs total) + `client/public/sitemap-blog.xml` **+1 URL lastmod=2026-08-30 priority=0.8 changefreq=monthly** (54 URLs total) ; les 2 XML sont well-formed (`xml.etree.ElementTree` parse OK). **Témoins R8 mesurés en Python sur disque** (cf. `~/.hermes/cache/blocked-scripts/check_t75054b90.py`) : `WORDS main=3070` (largement > +200 demandés par brief) ; `H1=1` query-first + `H2=12` + `H3=20` + `FAQ <details>=8` ; query exacte `vídeo porteiro 2 fios` = **32 occurrences** ; `como instalar` = **13 occurrences** ; `2 fios` = **100 occurrences** ; `€` mentions = 16 ; `Z[1-6]` = 6 ; `JSON-LD 5/5 valide` (`json.loads` strict OK) ; `R12 forbidden pronom = 0` (8 motifs vérifiés : `je suis`, `mon entreprise`, `sozinho`, `contato pessoal`, `falar comigo`, `contacte-me`, `minha empresa`, `você mesmo` = 0 hit chacun) ; `R-canon forbidden = 0` (6 motifs : `orçamento gratuito`, `orçamento grátis`, `deslocação gratuita`, `deslocação grátis`, `visita gratuita`, `visita grátis` = 0 hit chacun) ; `R145 delai chiffres = 0` (6 motifs : `\d+\s*min`, `\d+\s*h`, `24/7`, `24 horas`, `imediato`, `piquete` = 0 hit chacun) ; **NAP `+351 932 321 892` ×2 display** + `tel:+351****1892` ×3 (canon E.164 mascado conforme PRICING.md) + `wa.me/351932321892` ×2 ; **DGEG TRIESP n.º 90062** ×1 ; `Lei n.º 14/2015` ×3 ; `70 €/h` ×5 PRICING.md ; `15 € a 65 €` ×5 PRICING.md ; `130 km` ×1 PRICING.md ; `+50%` ×2 PRICING.md ; `maillage <main>` **11 liens internes** tous valides (`git show origin/main:client/public/<slug>.html >/dev/null` confirme existence des 11 slugs sur origin/main) ; `lang=pt-PT` ; UTF-8 valide, 0 CJK, 0 mojibake (`Ã`), `€` préservé ; chars/bytes = 40468/41359 (ratio cohérent). **Conformité doctrine** : R1 (push Git only), R4 (zéro invention — toutes les valeurs monaires viennent de `PRICING.md` verrouillé + DGEG source-of-truth + « mediante confirmação » pour variable = pattern doc validé 7+ fois dans cet HISTORIQUE), R5 (géo-neutre — « Trás-os-Montes » = modificateur géographique, 0 streetAddress, 0 claim local non vérifiable, les 4 pages `instalacao-videoporteiro-{ville}` sont des pages cluster légitimes pSEO), R7 (PR DRAFT en attente GO Filipe, **0 merge ce run**), R8 (témoins mesurés sur disque en Python : 5/5 JSON-LD + 11 liens valides + R145=0 + R12=0 + R-canon=0 + PRICING.md verbatim + NAP cohérent), R11 (`lang=pt-PT` intact, 0 marqueur pt-BR vérifié sur 10 motifs), R12 §12 (collectif : `a nossa equipa` ×1, `contacte-nos` ×1, `emitimos` ×1 ; 0 pronom interdit), R145 (0 délai chiffré post-purge), R-canon-2026-08-11 (0 formulation bannie), AGENTS.md §10 (robots.txt non touché), AGENTS.md §12 (NAP `+351 932 321 892` cohérent 4 sites Norte Reparos), AGENTS.md §13 (DGEG/Ficha+Termo `350 €` NON mentionné ici — la page vídeo-porteiro 2 fios est une **installation BT standard ≤ 41,4 kVA** qui peut nécessiter Ficha+Termo si remodeling profond, mais ne fait pas de claim chargeur VE/wallbox). **Branche / worktree** : `feat/enr-rankpush-video-porteiro-2-fios-t_75054b90` (worktree dédié `.worktrees/enr-rankpush-video-porteiro-2-fios-t_75054b90/`, créé par run #1), basée sur `origin/main` (= `29f95c9490` = post-PR #401). **HOTSPOT cross-task à surveiller** : PR #391 (`como-instalar-tomada-simples`) sur branche `feat/enr-conformidade-pt-pt-como-instalar-tomada-simples-t_3e689f3a` (distincte) — pas de collision car fichiers distincts, sémantiquement orthogonales (tomada simple Schuko 16A vs vídeo porteiro 2 fios bus). **HOTSPOT DOC** : cette entrée remplace le brouillon inséré par run #4735 (qui contenait des claims non vérifiables sur « 3 bugs critiques fixés » — inexact vu l'audit run #4737 qui montre 5/5 JSON-LD valides + R145=0 dès le départ dans la version on-disk récupérée). **Mesure d'impact attendue** (`gsc-trajectoire-cron.sh` J+7/J+14/J+28) : J+7 si pos passe < 5 ET impr 28j > 25 → ✅ win capturé (page dédiée query-first + 5 JSON-LD valides + 11 maillage interne + 2 sitemaps sync) ; J+14 si clics 28j ≥ 3 ET CTR > 10 % → ✅ capture confirmée ; J+28 si pos reste > 10 ET clics 28j = 0-1 → ⚠️ Rollback possible (la page ne capture pas la query malgré le query-first, diagnostic probable = autorité cluster faible sur cette niche spécifique ou cannibalisation avec PR #360 `instalar-videoporteiro`). **Gating R7** : **0 merge, 0 push prod, PR DRAFT à ouvrir** — STOP validation Filipe obligatoire. | R4 (zéro invention — PRICING.md verbatim + DGEG verrouillé), R7 (PR DRAFT en attente GO), R8 (témoins Python sur disque : 5/5 JSON-LD + 11 liens valides + R145=0 + R12=0 + R-canon=0 + maillage cohérent + UTF-8 OK), R11 (lang=pt-PT intact), R12 §12 (collectif intact, 0 pronom interdit), R145 (0 délai chiffré), R-canon-2026-08-11 (0 formulation bannie), AGENTS.md §10/§12/§13 (NAP + DGEG + 4 sites cohérent, chargeur VE hors scope), doctrine t_75054b90 L#001 (HOTSPOT spillover orphan run #1 récupéré via worktree dédié), leçon memory 27/08 + 28/08 (collisions hotspot cross-task — fichiers distincts, pas de merge conflict) | Diff : 4 fichiers créés/modifiés : (1) `client/public/blog/como-instalar-video-porteiro-2-fios.html` **créé** (241 lignes, 41 359 octets, 3070 mots main, 5 JSON-LD valides) ; (2) `public/blog/como-instalar-video-porteiro-2-fios.html` **créé** (mirror per t_ee6ab70e HOTSPOT pattern, byte-identique à (1)) ; (3) `public/sitemap.xml` **+1 ligne** (lastmod 2026-08-30 priority 0.7, total 3891→3892 URLs) ; (4) `client/public/sitemap-blog.xml` **+1 ligne** (lastmod 2026-08-30 priority 0.8 changefreq monthly, total 53→54 URLs) + cette entrée SEO_PLAN.md (replace le brouillon inexact du run #4735). Branche `feat/enr-rankpush-video-porteiro-2-fios-t_75054b90` depuis `origin/main` (= `29f95c9490`). **PR DRAFT à ouvrir** avec `gh pr create --draft --base main --head feat/enr-rankpush-video-porteiro-2-fios-t_75054b90 --title '[T3-INFO] enr : page dédiée como instalar vídeo porteiro 2 fios (GSC pos=6.6, 20 impr/1 clic 28j) [DRAFT]'`, attente GO Filipe R7. | ⏸ **PR DRAFT à ouvrir** — STOP merge Filipe (R7, mesure impact J+7 via gsc-trajectoire-cron.sh : pos<5 win, pos>10 rollback) |
