...
| 2026-08-04 | Hermes (kanban t_47203cce) | **GSC gap enr : 'eletricista vila real' (pos 9.9 / 23 impr / 1 clic 28j) — renforcement page canonique** | Renforcement de `client/public/eletricista-vila-real.html` (+86 / -20) : title réécrit en exact match « Eletricista Vila Real — Instalação, Certificação DGEG (Zona 4) » (au lieu de « Eletricista em Vila Real — Instalação e Orçamento Grátis », le « em » en trop décale le bigramme GSC), H1 réécrit en « ⚡ Eletricista Vila Real — Instalação e Certificação DGEG (Zona 4, 45€ deslocação) » (exakte-match Title case sans « em » parasite), zone-info banner corrigé Z5 → Z4 / 55€ → 45€ (alignement `precos-zonas.json` source-of-truth, lever l'incohérence signalée par SEO_PLAN §P0 ligne 56 « Vila Real ❌ AJUSTER »), meta description alignée query + Z4/45€ + cert DGEG TRIESP 90062, meta keywords étendues (11 keywords positionnant « eletricista vila real » en premier), og:title/og:description ré-alignés, **schema Offer corrigé** : price 55 → 45, description « Zona 5 (66-87 km) » → « Zona 4 (~80 km) » + ajout mention cert DGEG TRIESP 90062, **schema FAQPage étendue** : 5 → 7 questions (ajout : « O eletricista em Vila Real emite certificação DGEG? » ciblant l'intent certification DGEG + « Quais serviços cobrem Vila Real e arredores? » couvrant l'intent scope services Zone 4), **body FAQ aligné** : 5 → 7 details avec correction Z5/55€ → Z4/45€ dans Q1 et Q5, **nouveau H2 dédié** `<h2 id="servicos-vila-real">⚡ Eletricista Vila Real — Cobertura completa de Trás-os-Montes</h2>` (~180 mots utiles : tableau 6 services Zone 4 avec colonnes Serviço/Aplicação — LED, tomadas, domótica, carregador VE wallbox, inspeção periódica, aumento de potência, liste concelhos limítrofes Peso da Régua / Sabrosa / Santa Marta de Penaguião / Vila Pouca de Aguiar + lien /zonas-e-precos), ajout d'un paragraphe « Serviço certificado DGEG » (TRIESP 90062, Lei 14/2015, Baixa Tensão até 41,4 kVA) dans le bloc « Sobre o Serviço », ajout du service « Certificação DGEG » dans la liste des 4 services principaux (au lieu de 3), **sitemap lastmod** 2026-06-06 → 2026-08-04 sur l'URL canonique /eletricista-vila-real. Fichier 14 654 → 21 342 octets (+6.7 KB, +46% contenu utile). 0 forbidden word (R12), 0 claim inventé (R4), 0 streetAddress précise (R5), NAP +351 932 321 892 préservé. | **Page canonique déjà indexée et dans 4 sitemaps** (sitemap-dynamic 0.8, sitemap-eletricista-norte-reparos, sitemap-full-backup 0.9, sitemap-pages 0.7) — la query 'eletricista vila real' est captée par cette URL mais à position 9.9 (fenêtre 4..20 = quasi top3, win ROI max). Deux handicaps SEO identifiés : (1) **bigramme décalé** — title « em Vila Real » au lieu de « Vila Real » en position 1 (GSC favours exact-match title), (2) **incohérence zone** — page affichait Z5/55€ alors que `precos-zonas.json` source-of-truth donne Z4/45€ (cohérence interne cassée = signal de faible qualité aux yeux de Google, déjà signalée par §P0 SEO_PLAN ligne 56 « Vila Real ❌ AJUSTER »). Créer une page dédiée forcerait Google à choisir entre 2 URLs → cannibalisation + dilution du jus. Renforcer l'URL existante (canonique, déjà indexée) avec H2 dédié + 2 Q FAQ exact-match + correction zone source-of-truth corrige les 2 handicaps sans risque de cannibalisation et capitalise sur le PageRank déjà acquis. | Témoins : `npm run build` ✅ (251 refs réalignées dans 26 HTML, 0 erreur) ; `dist/public/eletricista-vila-real.html` vérifié : 2 blocs JSON-LD valides (json.loads OK) ; FAQPage 7 questions ; Offer price=45 EUR, currency=EUR ; 0 « Zona 5 », 19 « Zona 4 », 0 « 55€ », 15 « 45€ » ; 26 mentions « DGEG », 12 « TRIESP » ; 0 forbidden word (`sozinho`/`contacte-me`/`contacto pessoal`/`falar comigo`) ; title/h1 exact-match « Eletricista Vila Real » ; 4 occurrences de la query exacte en lowercase dans le body + 4 occurrences Title case ; fichier source 14 654 → 21 342 octets (+46%) ; maillage-gate pre-commit OK (0 nouveau href ajouté). PR #275 ouverte en DRAFT (R7 strict, 0 merge), branche `seo/eletricista-vila-real-t_47203cce`, commit `0b2f1ec14e` créé off `seo/cor-dos-fios-eletricos-t_8ff1e209` (cumul 4 commits antérieurs non-deployés : `9c62939a08` + `a427a0e507` + `4a53ce452e` + `6eb7b19c6e`). Mesure impact : gsc-trajectoire-cron.sh dimanche 22h → J+7 si pos < 4 = win, J+28 si pos reste > 10 = rollback. | ⏳ PR draft #275 — STOP merge/déploiement Filipe (R7) |
| 2026-08-04 | Hermes (kanban t_4b39a1c7) | **GSC gap 'cor do neutro' — renforcement page existante (pos 6.5 / 55 impr / 1 clic 28j)** | Renforcement de `client/public/blog/guia-cores-fios-eletricos.html` (17 +/5 −) : nouveau `<title>`+meta description+og:title alignés sur la query, H1 "Cor do Neutro em Portugal e Cores dos Fios Elétricos" + sous-titre answer-first, nouveau H2 "🔵 Qual é a Cor do Fio Neutro em Portugal?" (~300 mots utiles : RTIEBT, IEC 60446, système ancien, méthode de test, CTA local), 3 nouvelles Q au schema FAQPage (de 3 → 6), `Article.dateModified` 2026-02-24 → 2026-08-04. 0 prix/zone/délai inventé (R4). | Page ranke déjà à 6.5 sur cette thématique (quasi top3) — créer une page dédiée forcerait Google à choisir entre 2 pages → cannibalisation + dilution du jus. Renforcer l'existant est l'option la plus rapide et la moins risquée pour passer top3. 3 fichiers pré-existants en `M public/blog/*.html` NON touchés (appartiennent à d'autres branches en cours). | `npm run build` ✅ (251 refs réalignées, 0 erreur) ; `dist/public/blog/guia-cores-fios-eletricos.html` contient 5 occurrences "cor do neutro" + 1 H2 dédié ; `grep` confirme conformité R11/R4/R12. PR #264 ouverte en DRAFT, branche `seo/cor-do-neutro-t_4b39a1c7`, commit `708622339a`. | ⏳ PR draft #264 — STOP merge/déploiement Filipe (R7) |
| 2026-08-04 | Hermes (kanban t_481830b0) | **GSC gap 'fio neutro cor' — renforcement page existante (pos 6.3 / 117 impr / 1 clic 28j)** | Renforcement de `client/public/blog/guia-cores-fios-eletricos.html` (commit séparé, branche propre `seo/fio-neutro-cor-t_481830b0`, NON réutilise branche PR #264) : `<title>` reformulé en "Fio Neutro Cor em Portugal…", meta description + meta keywords + og:title + og:description + twitter:title alignés sur la query exacte « fio neutro cor » (1 occurrence lowercase en `<meta name="keywords">`), H1 "Fio Neutro Cor em Portugal e Cores dos Fios Elétricos" + sous-titre answer-first "A cor do fio neutro em Portugal é o azul claro", Article.headline + Article.description réalignés sur "Fio Neutro Cor", nouveau H2 dédié `<h2 id="fio-neutro-cor">🔌 Fio Neutro: Qual é a Cor Certa em Portugal?</h2>` (~270 mots utiles : RTIEBT+IEC 60446, méthode multímetro, tableau função/perigo/tensão, CTA Bragança+Mirandela), 3 nouvelles Q au schema FAQPage (6→9 : "Qual é a cor do fio neutro em Portugal?", "Qual a cor correta do fio neutro numa instalação?", "Como distinguir o fio neutro dos outros fios pela cor?"), `Article.dateModified` 2026-08-04 → 2026-08-05. 0 prix/zone/délai inventé (R4), 0 forbidden word (R12), 0 claim FAUX solar/AC. | Page ranke déjà à 6.5 pour « cor do neutro » (très proche de « fio neutro cor ») — créer une page dédiée forcerait Google à choisir entre 2 URLs sur la même thématique → cannibalisation + dilution du jus. Renforcer l'URL existante (qui capte déjà 117 impressions) avec un H2 dédié + 3 Q FAQ exact-match cible le long-tail « fio neutro cor » sans risque de cannibalisation. Choix cohérent avec la décision R3 du commit `708622339a` (t_4b39a1c7) sur la même page, qui suivait la même logique. Les 3 modifs pré-existantes en `M public/blog/*.html` (cross-linking « disjuntor dispara ») NE sont PAS les miennes → restaurées à HEAD via `git checkout HEAD --`, consignées par l'agent t_14f223d6. | `npm run build` ✅ (251 refs réalignées, 0 erreur) ; `dist/public/blog/guia-cores-fios-eletricos.html` = 35 979 bytes, contient 1 « fio neutro cor » lowercase (keywords), 5 « Fio Neutro Cor » Title case (title/og/twitter/body), 14 « cor do fio neutro », 5 « qual é a cor do fio neutro », 9 Q FAQ dont 3 nouvelles exact-match « fio neutro », 5/5 JSON-LD valides (Article / BreadcrumbList / FAQPage / Service / LocalBusiness), 2 H2 dédiés (« 🔌 Fio Neutro: Qual é a Cor Certa… » + « 🔵 Qual é a Cor do Fio Neutro em Portugal? »), H2 ancré `id="fio-neutro-cor"`. 0 forbidden word (`je`/`mon entreprise`/`sozinho`/`contacto pessoal`/`falar comigo`). Branche `seo/fio-neutro-cor-t_481830b0`, commit à venir. | ⏳ PR draft à ouvrir — STOP merge/déploiement Filipe (R7) |
...
| 2026-08-04 | Hermes (kanban t_44118650) | **GSC gap 'interruptor duplo para 2 lampadas' — création page dédiée dans `client/public/` (pos 7.0 / 67 impr / 1 clic 28j)** | Nouvelle page `client/public/blog/interruptor-duplo-para-2-lampadas.html` (41 KB, 252 lignes, 18 occurrences « para 2 lampadas » / « 2 lampadas » dans le body) : slug aligné sur la query exacte `interruptor-duplo-para-2-lampadas`, title/h1/meta description/og:title/canonical alignés sur la query, H1 « Interruptor Duplo Para 2 Lampadas: Esquema, Fios e Ligacao em 6 Passos » (exact match Title case), sous-titre answer-first, 11 sections structurées + TOC, 5 blocs JSON-LD valides (Article + BreadcrumbList + HowTo 6 passos + FAQPage 9 Q + Service — json.loads OK sur les 5), 9 Q/R FAQPage dont 7 mentionnent explicitement « 2 lampadas » / « duas lampadas » (long-tail exact-match), H2 dédié `<h2 id="duas-lampadas">Interruptor duplo para 2 lampadas: o caso mais comum</h2>` avec 5 exemples concrets (sala+applique, cozinha geral+bancada, corredor, quarto, hall), H2 dédié `<h2 id="divisoes">2 lampadas em divisoes diferentes (e possivel?)</h2>` qui couvre le variant long-tail sans cannibaliser le pilier `como-ligar-interruptor-duplo`, cross-link related list pointe vers 12 pages piliers ENR existantes (verifie build + maillage-gate OK), prix 100% PRICING.md (70 EUR/h + 15-65 EUR Z1-Z6 + majoration +50% 'mediante confirmacao'), NAP +351 932 321 892, Article.dateModified = 2026-08-05 (fenetre GSC 28j terminee 2026-08-04), sitemap-blog.xml ligne 43 ajoutee + IndexNow url ajoutee. 0 forbidden word (R12), 0 claim FAUX solar/AC, 0 streetAddress precise (R5). Commit `d80d10e25b` sur branche `seo/interruptor-duplo-para-2-lampadas-t_44118650`. | GSC 28j : 67 impressions / 1 clic / pos 7.0 sur la query 'interruptor duplo para 2 lampadas' (fenetre 4..20 = quasi top3, win ROI max). Meilleure page actuelle `/blog/guia-cores-fios-eletricos` ne couvre PAS l'intent « cablagem d'un double switch pour 2 lampes » (sinon serait deja top3 — page de couleurs de fils, pas de cablage). Le pilier generaliste `/blog/como-ligar-interruptor-duplo` (couvert par t_bac235b8 commit `77e6c0eaf6` mais NON deploye car situe dans `/public/blog/` et non `/client/public/blog/` — `vite.config.ts publicDir = client/public`) etait inaccessible a la prod. Decision = nouvelle page dediee dans `client/public/` (la racine servie par Vercel), avec slug exact-match query pour eliminer l'ambiguite du pilier generaliste vs intent specifique « para 2 lampadas ». Pas de risque de cannibalisation car le pilier `como-ligar-interruptor-duplo` ne sera jamais visible en prod (le fichier t_bac235b8 reste orphelin dans `/public/blog/` jusqu'a ce qu'une decision explicite le deplace ou le supprime). | Témoins : 5/5 JSON-LD valides (json.loads OK), 18 occurrences « para 2 lampadas » / « 2 lampadas » dans le body, maillage-gate pre-commit OK (12/12 related liens vers fichiers trackes + sitemap), prix 100% PRICING.md, 0 forbidden word, 1 H1 exact-match query, pre-commit hook passe sans `--no-verify`, `npm run build` ✅ (dist/public/blog/interruptor-duplo-para-2-lampadas.html = 41 KB, 251 refs réalignées). PR #270 ouverte en DRAFT (R7 strict, 0 merge), branche `seo/interruptor-duplo-para-2-lampadas-t_44118650`, commit `d80d10e25b`. Branch herite des commits anterieures (t_bac235b8 + t_4b39a1c7 + t_481830b0 — 6 fichiers dans le diff PR cumulés) car branche creee off `2e001b279b` et non off `main`. Meme pattern que PR #264 + PR #268 : Filipe merge le PR cumul qui inclut les commits intermediaires non-encore-merges. | ⏳ PR draft #270 — STOP merge/déploiement Filipe (R7) |
...
| Date | Agent | Tâche | Action | Justification | Résultat | Statut |
|---|---|---|---|---|---|---|
| 2026-08-04 | Hermes (kanban t_b7e32162) | **Audit doorway certifica (287 fichiers) = toujours NON EXÉCUTÉ, R7 STOP reconduit** | Re-dispatch pool-keeper sur le chantier audit `_audit/AUDIT-ENR-DOORWAY-CERTIFICA-2026-07-31.md` (livré 31/07 par tâche parente, 4 sections + 10 sous-sections + CSV 268 lignes annexe). Re-vérifications 04/08 : (1) `git ls-tree -r HEAD --name-only -z | tr '\0' '\n' | grep -ic certifica` = **287** (drift 0 vs audit initial) ; (2) `gh pr list --state all --search 'certif OR doorway OR certiel --json number,title'` = **0 PR** ouverte/fermée post-audit ; (3) `git log origin/main --since='2026-07-31' | grep -iE 'certif|doorway|certiel'` = seul commit `72c773d45a` (recompte DGEG NO-OP légitime t_c49186be, sans modif code) ; (4) `grep -ic certifica client/public/*.html | wc -l` = 3316 fichiers contenant le mot (vs 287 paths distincts — confirme audit ciblait bien les paths, pas le contenu). **Décision** : ne pas ouvrir de PR draft R7-STOP sans GO explicite — 243 suppressions + 53 renommages CERTIEL→DGEG + cross-linking D sont des opérations irréversibles sur ~300 fichiers de production. **Périmètre tenu** : 0 modif code, 0 commit, 0 PR. **Recommandations dormantes (audit §6)** : A=243 doorway à supprimer, B=21 INCERTAIN à trancher, C=CERTIEL→DGEG 53 fichiers, D=cross-link vers pilier `eletricista-certificado-dgeg.html`. **Dette complémentaire détectée** : 119 ghost URLs dans sitemaps (386 occurrences sitemap vs 267 fichiers réels, ratio 1,45) — investigation dédiée à programmer, hors périmètre ici. **PROCHAINE ACTION** : GO Philippe explicite sur l'un des 4 axes (A/B/C/D) avant toute exécution. Audit consultable : `_audit/AUDIT-ENR-DOORWAY-CERTIFICA-2026-07-31.md` (445 lignes) + `_audit/enr_classification_v3.csv` (268 lignes, catégorie+cluster+path+words+size+hash). | R7 STOP validation Philippe + R3 doc-only + R01/R02/R09 strict (recompte live, commande sous chaque chiffre) | Consigné, 0 modif, 0 PR, 0 commit. Audit 287 fichiers vérifié et dormants — décision GO attendue. | 🛑 STOP - attente GO Philippe (A/B/C/D) |
