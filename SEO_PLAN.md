...
| 2026-07-30 | cowork-loop | **R12 (priorité R11/R12) — purge des mots-clés urgence/24h dans `shared/siteConfig.ts` → `seo.keywords`** | Violation R12 ACTIVE en prod détectée en lecture pré-B4. §RÈGLES DU PROJET de ce fichier stipule « Pas de "urgente" ici », or `seo.keywords` ciblait **9 requêtes d'urgence**, injectées en `<meta name="keywords">` sur **toutes les pages** (`client/src/components/SEOHead.tsx` L35, `SEOHeadEnhanced.tsx` L29) → cannibalisation de l'intent `eletricista-urgente.pt`. Retirés : `eletricista urgente bragança`/`macedo de cavaleiros`/`chaves`, `eletricista 24 horas mirandela` (×2, aussi un doublon), `curto-circuito urgente`, `avaria elétrica urgente bragança`, `eletricista emergência 24h`, `avaria elétrica urgente domingo`. Remplacés par des équivalents installation issus de `siteConfig.services[]` (Instalação Elétrica, Quadro Elétrico, Iluminação LED, Reparação Avaria Elétrica). **Aucun mot-clé `certificação`/DGEG/CERTIEL introduit** (PR #235 `fix/dgeg-certiel-removal` en cours). **Même gisement que celui corrigé sur `canalizador-norte-reparos` (PR #203, 16/07) mais jamais traité ici.** | R11/R12 (violation détectée = priorité sur la tâche prévue), R4 (zéro invention — vocabulaire repris de `services[]`), R8 (témoins avant/après), commit atomique 1 fichier = 1 commit, R7 (zéro merge) | Témoins R8 sur `seo.keywords` : AVANT 41 entrées / 39 uniques (2 doublons) / **9 violations R12** → APRÈS 41 entrées / **41 uniques (0 doublon)** / **0 violation R12**. `npx tsc --noEmit` : 0 erreur sur `shared/siteConfig.ts` (82 erreurs pré-existantes ailleurs, inchangées). Branche `loop/2026-07-30-eletricista-norte-reparos-r12-seo-keywords` depuis `origin/main`. | ⏳ PR ouverte — attente GO merge Philippe (R7) |
