...
| 2026-08-28 | Hermes (kanban t_3e689f3a) | **[CONFORMIDADE-URGENT] enr : `public/blog/como-instalar-tomada-simples.html` — pt-br « você mesmo » → pt-pt « por si próprio »** | Fichier **untracked** détecté (créé localement sur la branche `feat/enr-rankpush-montar-interruptor-duplo-t_54bccc70`, non commité). **Scan exhaustif des marqueurs pt-br** sur le fichier (regex étendue : `você | a gente | tá isolée | pra | brilhando | ficou | estamos te `) = **1 seul hit** : ligne 174, section `<section id="quando">` H2 « Quando chamar eletricista habilitado em vez de fazer **você mesmo** » (registre brésilien non naturel au Portugal). **Fix pt-pt neutre** : « Quando chamar eletricista habilitado em vez de fazer **por si próprio** » (formulation inclusive standard pt-pt, alignée avec « a nossa equipa / contacte-nos » du reste du site). **Source-of-truth-first vérifié** : (a) PRICING.md L7 « 70 €/h » + L14-19 « Z1-Z6 15/25/35/45/55/65 € » + L25 « Ficha + Termo a partir 350 € » = verbatim préservés ; (b) « doctrina de transparencia radical » L192 = terme technique verrouillé PRICING.md L41 (et non gallicisme — traduction du terme interne) ; (c) « bricolage » (emprunt courant pt-pt, déjà utilisé dans `blog-como-montar-uma-tomada.html` L37+174) préservé tel quel. **Aucun autre risque** : (1) **0 invention** R4 (1 mot -1/+1, substitution exacte, aucun prix/zone/délai/service ajouté) ; (2) **Conformité R12 §12** : 0 « je / sozinho / falar comigo / mon entreprise » (rôle PME préservé « a nossa equipa »), 0 « chargeur VE / DGEG inventé » (cf. AGENTS.md §13) ; (3) **NAP préservé** : `+351 932 321 892` E.164 dans CTA L216 + footer L235, 0 contamination `928 484 451` (numéro plomberie) ; (4) **Aucun régression SEO** : H1, meta description, og, twitter, canonical, 5 blocs JSON-LD (Article + BreadcrumbList + HowTo + FAQPage + Service) inchangés. **Isolation spillover orphan** (leçon `t_54bccc70` 28/08) : fichier créé sur branche t_54bccc70 NON commité → isolation sur `feat/enr-conformidade-pt-pt-como-instalar-tomada-simples-t_3e689f3a` depuis `origin/main`, worktree `.worktrees/t_3e689f3a`, **0 pollution** du checkout partagé sur t_05d17f5d (`git status --short` final = vide). **Commit atomique** : `908175b452` (1 fichier, 241 insertions / 0 suppressions sur compteur Git car fichier nouveau, -1/+1 sur le contenu de la substitution stricte). **Push + PR draft** : branche pushée sur `origin/feat/enr-conformidade-pt-pt-como-instalar-tomada-simples-t_3e689f3a`, **PR #391** ouverte en DRAFT (https://github.com/taffrand-gif/eletricista-norte-reparos/pull/391), **baseRefName=main**, **additions=241, deletions=0, changedFiles=1**. **Vérifications outillage** : grep étendu final = 0 « você », 0 « a gente », 0 « ta » isolé (les « tá » restants = fragments de « está », verbe « estar » 3ᵉ personne, OK pt-pt), 0 « cê / pra / brilhando / ficou ». **GATE R7 — STOP, attente GO Filipe** : PR #391 en draft, 0 merge, 0 auto-merge (R7 verrouillée). | R4 (zéro invention : 1 substitution de mot, aucun prix/zone/délai/service), R8 (témoin : ligne 174 « você mesmo » 1→0, « por si próprio » 0→1), R12 (0 pronom 1ère pers, registre PME préservé), R13 (chargeur VE non concerné — aucun claim ajouté), R6/R7 (PR draft, 0 merge sans GO), R11 (consignation HISTORIQUE = présente) |
