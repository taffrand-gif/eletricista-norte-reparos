...
| 2026-08-04 | Hermes (kanban t_596e7ff5) | **GSC gap enr : 'cores dos fios elétricos' (pos 5.8 / 73 impr / 1 clic 28j) — alias plural de la tâche t_c302be14** | Confirmation que la query 'cores dos fios elétricos' (avec accent + pluriel) est couverte par les 2 commits déjà mergés sur la branche `seo/cores-dos-fios-eletricos-t_596e7ff5` (t_4b39a1c7 + t_c302be14) : title contient « Cores dos Fios Elétricos » (mot exact), meta description commence par « Cores dos fios elétricos em Portugal », meta keywords positionnent « cores dos fios elétricos » en premier, og:title et twitter:title alignés, Article JSON-LD `headline` + `dateModified=2026-08-04`, FAQPage étendue à 5 Q (3 génériques + 2 ciblées : « Quais são as cores dos fios elétricos em Portugal? » + « Qual é o código de cores dos fios elétricos? »), H2 ancré `id="cores-dos-fios-eletricos"` avec tableau récap + mnémonique + paragraphe pré-2000. | Cette tâche `t_596e7ff5` (query 'cores dos fios elétricos' avec accent + pluriel) couvre le même intent que `t_c302be14` (query 'cor dos fios eletricos' sans accent + singulier) : Google Search Console les traite comme la même famille de requêtes (variantes stemming). Plutôt que de créer une page dupliquée qui cannibaliserait, on capitalise sur le travail déjà livré dans PR #272 (même branche, commits `06a1b2edee` + `e9884bce21`). Vérifié : 11 occurrences de la query pluriel exacte dans le fichier, 5/5 blocs JSON-LD valides (json.loads OK), build vert (251 refs réalignées, 0 erreur). | Témoins : `npm run build` ✅ ; `grep -ciE 'cores dos fios elétricos' dist/public/blog/guia-cores-fios-eletricos.html` = 11 ; `python3 -c "import json; [json.loads(s) for s in re.findall(...)]"` 5/5 OK ; PR #272 DRAFT sur branche `seo/cores-dos-fios-eletricos-t_596e7ff5` (base=main), commits cumulés `06a1b2edee` + `e9884bce21` ; file `client/public/blog/guia-cores-fios-eletricos.html` = 44 lignes, 35 047 octets ; 0 forbidden word (R12), 0 claim inventé (R4), 0 streetAddress précise (R5), NAP +351 932 321 892 préservé. | ⏳ PR draft #272 — STOP merge/déploiement Filipe (R7) — mesuré à J+7 via gsc-trajectoire-cron.sh |
| 2026-08-04 | Hermes (kanban t_10aa11cf) | **GSC gap enr : 'disjuntor fazendo barulho é perigoso' — création page dédiée dans `client/public/` (pos 6.5 / 55 impr / 1 clic 28j)** | Nouvelle page `client/public/blog/disjuntor-fazendo-barulho-e-perigoso.html` (36 KB, 231 lignes) : slug aligné sur la query exacte `disjuntor-fazendo-barulho-e-perigoso`, title/h1/meta description/og:title/canonical alignés sur la query, H1 « Disjuntor Fazendo Barulho É Perigoso? 5 Causas e Quando Chamar Eletricista » (exact match Title case), 11 sections structurées + TOC, 5 blocs JSON-LD valides (Article + BreadcrumbList + HowTo 5 passos + FAQPage 9 Q + Service — json.loads OK sur les 5), 9 Q/R FAQPage couvrant les 5 causes (sobrecarga, mau contacto, componente degradado, curto-circuito incipiente, fuga de corrente), tableau récap type-de-barulho/cause/gravidade/acção, tableau comparatif disjuntor-do-quadro vs disjuntor-de-tomada, cross-link related list pointe vers 22 pages piliers ENR existantes (0 lien 404 vérifié), prix 100% PRICING.md (70 EUR/h + 15-65 EUR Z1-Z6 + majoration +50% 'mediante confirmação'), NAP +351 932 321 892 non masqué (tel:+351****1892 interdit corrigé), liens /contacto → /contactos (cohérent vercel.json), mentions outils spécifiques (FLIR/Fluke T6-1000/megger MFT1741+) → formulation générique « câmara térmica + multímetro profissional + megóhmetro » (R4 zero invenção), Article.dateModified = 2026-08-05 (fenetre GSC 28j terminee 2026-08-04), sitemap-blog.xml ligne 48 ajoutée + IndexNow url ajoutée. 0 forbidden word (R12), 0 claim FAUX solar/AC, 0 streetAddress précise (R5). | GSC 28j : 55 impressions / 1 clic / pos 6.5 sur la query 'disjuntor fazendo barulho é perigoso' (fenetre 4..20 = quasi top3, win ROI max). Meilleure page actuelle `/blog/guia-cores-fios-eletricos` ne couvre PAS l'intent « barulho de disjuntor » (page de couleurs de fils). La page existante `/blog/quadro-eletrico-barulho-causa` parle du quadro électrique général, pas spécifiquement du disjuntor. Décision = nouvelle page dédiée dans `client/public/` (la racine servie par Vercel), avec slug exact-match query pour eliminer l'ambiguite du pilier generaliste vs intent specifique « disjuntor fazendo barulho ». Pas de risque de cannibalisation car la page quadro-eletrico-barulho-causa cible l'intent « quadro fazendo barulho » (plus large, incluant diferencial/barramento), pas l'intent « disjuntor faisant du bruit ». Incident : ma première version de la page a été écrasée pendant la tâche par un autre worker (worktree parallèle), j'ai récupéré la version écrasée et appliqué 4 correctifs (tél masqué → non masqué, /contacto → /contactos, 2 liens related cassés → corrigés, mentions outils spécifiques → formulation générique) avant commit. | Témoins : 5/5 JSON-LD valides (json.loads OK), 14 occurrences « disjuntor fazendo barulho » + 4 occurrences « disjuntor a fazer barulho » dans le body, 22 related liens vers fichiers trackés (0 manquant sauf /styles.css qui existe), prix 100% PRICING.md, 0 forbidden word (`contacte-me`/`sozinho`/`falar comigo` — seul « sozinho » apparaît dans un slug URL de related link `/blog/diferencial-dispara-sozinho-causa`, pas dans le contenu rédigé), 1 H1 exact-match query, pré-commit hook passe, `npm run build` ✅ (251 refs réalignées, 0 erreur). PR #273 ouverte en DRAFT (R7 strict, 0 merge), branche `seo/disjuntor-fazendo-barulho-e-perigoso-t_10aa11cf` (créée off `seo/cores-dos-fios-eletricos-t_596e7ff5`, commit `7206868ddd`). | ⏳ PR draft #273 — STOP merge/déploiement Filipe (R7) |
