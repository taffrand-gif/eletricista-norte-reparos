...
| 2026-08-31 | Hermes (mode loop) | **fix ENR §135 faux-services-reste (prototype)** | PR #418 draft — branche `fix/enr-faux-services-reste-t_15cb76b8` (commit 0495a56784, 3 fichiers +4/-4) : purge panneau solaire + ar condicionado + 2 FAQ solaire → reframed avec disclaimer DGEG TRIESP 90062 hors-périmètre (cf. DGEG-CERT-SOURCE-OF-TRUTH.md). Témoin R8 grep 'eletricista' stable = 46406. **Reste à traiter** : 12 pages dédiées `aquecimento-eletrico-*.html` + autres pages HORS (piso radiante, bomba de calor, SCE) — batch à venir après validation prototype. **Méthode** : prototype 3 fichiers avant batch (cf. SEO_PLAN §138). wallbox conservé (RÉEL post-cert). R4 zéro invention + R7 STOP validation Philippe respectés. | Kanban t_15cb76b8 (10h après timeout iteration budget session précédente) | ✅ PR draft ouverte, 0 merge sans GO | ⏸ En attente GO Philippe |
