...
| 2026-09-01 | Hermes | **GSC gap T3-INFO 'cabo neutro cor' R3** (t_21d3a9f3) | Renforcement page `client/public/blog/guia-cores-fios-eletricos.html` (pos 8.8 / 20 impr / 0 clic en 28j, fenêtre 4..20 = presque top3) — page canonique déjà TRÈS solide (PR #421 31/08 rank-push R1 'fios eletricidade cores' fermé via `fix/enr-gsc-gap-fios-eletricidade-cores-t_f20277a6`). Diagnostic R3 : la query bare « cabo neutro cor » (3 mots, signal sémantique « couleur du câble neutre ») matche imparfaitement la page : 17 mentions « fio/cabo/condutor neutro » mais 0 mention exacte « cabo neutro cor » dans H1/title/intro/keywords. Leviers appliqués : (a) **recadrage verbatim** `<title>` (62 chars) et `og:title`/`twitter:title` sur « Cabo Neutro Cor: Azul-claro RTIEBT 2026 + Tabela (Fase, Terra) » — alignement direct query + signal couleur principal (azul-claro) ; (b) **meta `keywords` étendue verbatim** avec « cabo neutro cor » + 9 variantes (« cor do cabo neutro », « cabo neutro azul », « cor do neutro », « fio neutro cor », « cor fio neutro », « neutro cabo », « cabo neutro RTIEBT », « cabo neutro IEC 60446 », « código cores cabo neutro ») — verrouillées R11 = aucune invention, toutes sont des variantes naturelles de la query ; (c) **H1 + intro `class="intro"`** recentrés : « Cabo Neutro Cor: Azul-claro RTIEBT 2026 + Tabela Completa (Fase, Terra) » + intro verbatim « O cabo neutro tem uma cor única e obrigatória em Portugal: azul-claro. Esta é a resposta direta à pesquisa « cabo neutro cor »… » — body « cabo neutro cor » 0 → 8 occurrences, réponse directe en intro ; (d) **+section H2 `id="cabo-neutro-cor"`** (~470 mots de contenu utile, ~200 mots au-dessus du minimum R3 200 mots) avec 3 H3 (« Por que é que o cabo neutro tem esta cor específica? » 4 raisons · « Como identificar a cor do cabo neutro numa casa portuguesa — 5 passos » méthode · « Casos especiais — quando a cor do cabo neutro não é azul-claro » 3 cas pré-2000/non-conformes/électro-ménager) + mnemónica « Azul Volta, Castanho Mata, Verde-Amarelo Protege » — toutes les couleurs sont définies par les normes déjà citées (RTIEBT/IEC 60446/HD 308 S2), 0 invention prix/zone/délai ; (e) **+2 FAQ JSON-LD verbatim** sur la query (Q11 « Cabo neutro cor — qual é a cor obrigatória em Portugal? » réponse azul-claro unique · Q12 « Cabo neutro cor azul ou outra? Como identificar o neutro em qualquer instalação? » réponse méthode 5 étapes) — 0 prix/zone/délai ajouté, uniquement informationnel (RTIEBT/IEC 60446/HD 308 S2 + DGEG TRIESP 90062 en redirection intervention) ; (f) **+2 FAQ visibles `<details>`** correspondantes dans la section FAQ (10 → 12 Q/R) ; (g) **+2 inbound link cluster** dans « Artigos Relacionados » : `/blog/cabo-eletrico-seccoes.html` (« Secções de cabos elétricos 1.5/2.5/4/6/10 mm² », signal sémantique « choisir le bon câble ») + `/blog/como-escolher-cabos-eletricos.html` (« Como escolher cabos elétricos em conformidade com o RTIEBT », signal sémantique RTIEBT explicite, anchor text renforce le cluster) — la page n'avait aucun lien vers le cluster « cabos » ; (h) **bump dateModified** JSON-LD 2026-08-31 → 2026-09-01 + visible body « Última atualização » 31 ago → 1 set 2026 + breadcrumb « Cabo Neutro Cor — Código de Cores dos Fios Elétricos » (signal freshness Google + alignement query). Aucune invention prix/zone/délai (block Transparence inchangé, sourcé PRICING.md : 70 €/h, Z1-Z6 15-65 €, ficha+termo 350 €, +50% majoration). PR draft à ouvrir depuis `origin/main` sur branche `fix/enr-rankpush-cabo-neutro-cor-r3-t_21d3a9f3`. Aucun merge sans GO (R7). Impact à mesurer à J+7 via `gsc-trajectoire-cron.sh` : cible pos < 4 (la page passe de bare-query dilution à query-first via `keywords` meta verbatim + 2 FAQ JSON-LD verbatim + section dédiée `cabo-neutro-cor` + cluster maillé + dateModified fresh). Rollback possible si position reste > 10 à J+28. | Gap GSC fenêtre 28j (pos 8.8 = page 1 basse → top3 = levier `keywords` meta verbatim + FAQ query-first + section dédiée + cluster maillé + dateModified fresh) | +1 title recadré / +1 H1 recadré / +9 keywords meta verbatim / +1 intro verbatim / +1 section H2 (~470 mots) / +2 FAQ JSON-LD verbatim / +2 FAQ visibles / +2 inbound link cluster / +1 dateModified fresh | ⏸ PR draft en cours, attente GO Philippe (R7) |
