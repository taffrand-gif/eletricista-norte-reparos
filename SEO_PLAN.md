...
## 🔄 HISTORIQUE
| 2026-08-25 | cowork-loop | **Rang 2 de la file — 6 des 13 pages `client/public/` à balises déséquilibrées, réparées ; et la CAUSE de la famille principale identifiée** | Contrôle des PR ouvertes passé **avant** calcul du périmètre (24 PR ouvertes, `gh pr view <n> --json files` sur les 24) : sur 13 pages déséquilibrées, **5 sont réservées** (`comparacao` #348, `certificacao-certiel-macedo-de-cavaleiros` #349, `servicos` + `galeria` #350, `blog/guia-cores-fios-eletricos` #362/#370) → **8 libres**, 6 traitées. 🔴 **Cause identifiée pour la famille principale : la chaîne de génération injecte le bloc de liens internes À L'INTÉRIEUR du `<h2>`, et sur 6 pages son `<div>` OUVRANT a disparu** — il ne reste que le `</div>` fermant. Prédicat exact `<h2…><a href="/areas-atuacao"` (wrapper absent) : **6 fichiers**, contre **1 seul donneur intact** (`sobre-mim.html`), dont le wrapper n'a **qu'une seule forme distincte** : `<div style="margin-top:10px;font-size:13px">`. Transplantation **byte-identique**, aucun texte modifié. 2 des 6 (`servicos`, `galeria`) réservées par #350 → 4 traitées. **Deuxième famille (2 pages, `equipa` + `recursos-gratuitos`)** : `<div>` laissé ouvert avant le bloc `related`. **Invariant remesuré ce run sur une population portée à 2843 pages** : profondeur `<div>` à `<section class="related">` = **0 sur 2843/2843**, et à `</main>` = 0 sur 1936/1936. Ces 2 pages étaient à 1 → `</div>` inséré **avant le bloc `related`**, position établie par l'invariant (avant `</main>` équilibrerait aussi mais violerait l'imbrication — leçon du 24/08 reconfirmée). **`garantias-e-seguros.html` portait DEUX défauts** (-2) : le wrapper **et** un `<div class="faq-item">` ouvrant disparu, dont les 3 autres `faq-item` de la même page donnent la forme exacte. ⚠️ **Défaut de contenu résiduel non traité (R4)** sur cette page : la **question** de ce bloc FAQ a été détruite et sa réponse commence par une prose tronquée — « **O** tem um número único… », le sujet effacé. **La reconstruire serait une invention → GO.** ⚠️ **Non traité, sans donneur** : `faq.html` (4 `</div>` orphelins, famille `faq-item` distincte) ; `instalacao-eletrica.html` (+1, page générée avec `data-loc`, structure différente) ; et le couple `distrito-*` porte **un second défaut de contenu** — `<div class="cta"><h2>` partiellement détruit, aucun donneur dans le dépôt. | R4 (zéro invention — un seul donneur mais **byte-identique** et de forme unique ; aucun texte modifié, uniquement des balises) ; R4 dans l'autre sens (3 défauts de contenu **signalés et non reconstruits**) ; R8 (témoin = équilibre `div`/`section`/`main`/`h1` compté **en Python sur le contenu**) ; commit atomique 1 fichier = 1 commit ; R6 ; R7 (zéro merge) ; R-WT (worktree `~/work/Sites/_worktrees/loop-enr-20260825` ; checkout partagé **non touché**) | **6 commits, 6 fichiers de production.** Diff : **6 insertions, 6 suppressions** — 6 lignes touchées, aucune modification de texte. **Témoin R8 par fichier** : `distrito-braganca` −1→0 · `distrito-vila-real` −1→0 · `parcerias` −1→0 · `garantias-e-seguros` −2→0 · `equipa` +1→0 · `recursos-gratuitos` +1→0. Contrôle croisé final sur les 6 : `div`, `section`, `main` et `h1` **tous équilibrés**. Branche `loop/2026-08-25-enr-div-desequilibre` depuis `origin/main`. | ⏳ PR ouverte |
