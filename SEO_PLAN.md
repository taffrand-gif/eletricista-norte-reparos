...
| 2026-08-04 | Hermes (kanban t_b6730f41) | **Audit GAP « eletricista 24 horas » (DFSEO CPC 12,66€ / vol 170)** | Vérification anti-doublon : `client/public/eletricista-24-horas.html` existe déjà et cible exactement la query dans title/H1/FAQ/canonical ; PR #259 est **MERGED**. | GAP GSC 0 impression / 0 clic ne signifie pas absence de couverture. Aucun nouveau pilier ni PR draft : duplication/cannibalisation à éviter. | **NO-OP légitime** : couverture existante confirmée ; 0 fichier code modifié ; mesure J+7 via `gsc-trajectoire-cron.sh`. | ✅ NO-OP — couverture déjà livrée (R7) |
| 2026-08-04 | Hermes (kanban t_90ba25d0) | **Re-audit GAP « eletricista 24 horas » (DFSEO CPC 12,66€ / score 2152,20)** | Re-vérification ciblée après pool-keeper re-injection : `find -iname 'eletricista-24-horas*'` confirme page canonique `client/public/eletricista-24-horas.html` (163 lignes) + `content/blog/eletricista-24-horas-guia-completo.md`. PR #259 « pilier eletricista-24-horas.html (KW jackpot 2152, 2 hits) » est **MERGED** 2026-08-03T16:48:54Z. Audit qualité de la page (titres, schema, NAP) : `<title>` + `<h1>` alignés sur query exacte, **FAQPage 7 questions** (5-9 cible OK) couvrant 24h/dispo/prix/zone/orçamento/DGEG/types avaria/majoração, `LocalBusiness` + `Service` + `Person` (Filipe TRIESP 90062) valides, NAP `932 321 892` correct (téléphone élec, pas celui du canalizador — R2 respectée), cross-link `/precos-eletricista` + `eletricista-urgente.pt` présents. Prix/zone DGEG strictement alignés sur PRICING.md (Z1=15€, Z6=50€, 70€/h, +50% noturno/feriado). Zéro invention R4. | GAP GSC persistant (0 impr/0 clic 28j) sur page mergée hier 03/08 = promotion lente page jeune, pas manque de couverture. 3e tentative successive sur le même chantier aujourd'hui (t_b6730f41 NO-OP, t_8024328a rank-push #223 DRAFT — PR #223 = fix téléphone, non liée). Ouvrir PR doublon = violation leçon anti-doublon (#469 cross-site) + risque cannibalisation `eletricista-24-horas.html` vs guide blog. | **NO-OP légitime (2e confirmation)** : couverture canonique validée ligne-par-ligne ; 0 fichier code modifié ; 0 PR draft ouvert ; consigne append-only. Mesure J+14 via `gsc-trajectoire-cron.sh` (recette win = pos < 4) ; si toujours GAP à J+14, prochaine action = levier CTR (title/meta A/B) sans dupliquer l'URL. | ✅ NO-OP — couverture déjà livrée + validée (R7) |
| 2026-08-04 | Hermes (kanban t_989c1d65) | **Re-audit GAP « eletricista 24 horas » (DFSEO CPC 12,66€ / vol 170 / score 2152,20) — 3e réinjection pool-keeper** | Re-vérification ciblée 3e passage sur la même query MONEY la plus chère du marché PT. Preuves 5 canaux : (1) `find -iname 'eletricista-24-horas*'` = 4 hits (`client/public/eletricista-24-horas.html` 163 lignes, `content/blog/eletricista-24-horas-guia-completo.md`, `public/sitemap-blog.xml`, `client/public/instalacao-eletrica.html` cross-link) ; (2) PR #259 « pilier eletricista-24-horas.html (KW jackpot 2152, 2 hits) » = **MERGED 2026-08-03T22:01:13Z** (mergedAt confirmé via `gh pr view 259`) ; (3) `curl -sI https://eletricista-norte-reparos.pt/eletricista-24-horas` = **HTTP/2 200** served by Vercel (`etag 8e33a3ea…`, `last-modified 2026-08-04T13:52:50Z`) — page LIVE en prod ; (4) audit qualité page : `<title>` + `<h1>` (`⚡ Eletricista 24 Horas em Trás-os-Montes`) alignés query exacte, `FAQPage` 7 questions (24h/prix/zone/orçamento/DGEG/types avaria/majoração) au-dessus du seuil cible, `LocalBusiness` + `Service` (7 offres Z1-Z6) + `Person` (Filipe TRIESP 90062) + `BreadcrumbList` valides, NAP `932 321 892` téléphone élec uniquement (R2 OK), 4 cross-links internes (`/precos-eletricista` + `/areas-atuacao` + `/carregador-veiculo-eletrico` + `eletricista-urgente.pt`), 1 cross-link externe (marque sœur) ; (5) prix strictement alignés PRICING.md (Z1=15€ → Z6=50€, 70€/h, +50% nuit/WE/feriado, Ficha+Termo a partir de 350€). Zéro invention R4, R5 (géo-neutre), R11 (DGEG scope strict), R12 («nossa equipa» x 8). | 3e réinjection pool-keeper sur même query = leçon anti-doublon t_90ba25d0 + t_b6730f41 déjà consignées. Ouvrir PR doublon = violation R7 + cannibalisation `/eletricista-24-horas.html` vs guide blog + collision avec PR #259 MERGED. Pattern leçon 04/08 : pool-keeper re-injection sans re-audit local = faux GAP signal. Mesure J+14 via `gsc-trajectoire-cron.sh` (recette win = pos < 4 + premières impressions > 0). Si toujours GAP à J+14, prochaine action = levier CTR (title/meta A/B) sans dupliquer l'URL, jamais re-créer de pilier. | **NO-OP légitime (3e confirmation)** : couverture canonique validée sur 5 canaux ; 0 fichier code modifié ; 0 PR draft ouvert ; consigne append-only. Leçon renforcée : pour query money déjà couverte par PR MERGED, le pool-keeper doit filtrer sur `mergedAt NOT NULL` avant de réinjecter, sinon même chantier revient toutes les ~10 min. | ✅ NO-OP — couverture PR #259 MERGED confirmée en prod (R7) |
| 2026-08-04 | Hermes (kanban t_b2d2b734) | **Audit SEO_PLAN ligne 259 « Levier CTR — title/meta `/eletricista-braganca` » : chantier déjà absorbé par PR #137 (MERGED 2026-07-05T21:28:32Z, SHA c3b7a67bb3) — pas de PR draft à ouvrir** | Vérification ciblée ligne 259 (statut affiché `⏳ PR ouverte — attente merge Philippe (R7, 0 merge main)`) : le chantier Levier CTR Bragança (commit `523715b34c` sur branche `fix/enr-ctr-braganca-title-meta`) a été **absorbé par PR #137 « fix(CTR,ENR): title+meta 5 villes-sèdes — hook orçamento grátis (levier striking) »** qui a étendu le pattern à 5 villes-sèdes (Bragança + Vila Real + Chaves + Mirandela + Macedo de Cavaleiros) et a été MERGED le 2026-07-05. Confirmation multi-canaux : (1) `git log -1 f66f5b5d20` (commit MERGE) ; (2) `gh pr view 137 --json state,mergedAt,mergeCommit` = `MERGED` / `2026-07-05T21:28:32Z` / `c3b7a67bb3` ; (3) `git show c3b7a67bb3 --stat` = 6 fichiers modifiés (SEO_PLAN.md + 5 pages villes-sèdes) ; (4) `grep "Orçamento Grátis" client/public/eletricista-braganca.html` = 1 hit dans le `<title>` (preuve de l'application du pattern) ; (5) `grep "Ligue já: +351 932 321 892" client/public/eletricista-braganca.html` = 1 hit dans la meta description (CTA call-intent). Bonus audit : (a) ligne 256 (PR #139 conformité batch) = MERGED 2026-07-06T10:58:53Z (commit `41e775e44b`, 7355 +/8243 -) ; (b) ligne 257 (PR #137 réplique 4 villes-sèdes) = MERGED 2026-07-05T21:28:32Z ; (c) ligne 259 (PR prototype Bragança) = absorbée par #137. Lignes 256, 257, 259 = toutes absorbées, statut `⏳` obsolète mais **NON corrigé en place** par principe append-only (modification du statut = réécriture d'historique, à faire par prochaine PR si besoin). | Protocole point 4 (chantier déjà fait = consigner dans SEO_PLAN ligne suivante + LECON, PAS de PR draft à ouvrir). R4 (zéro invention) + R7 (0 merge sans GO) + 03/08 protocole (audit avant exécution, ne pas ouvrir de PR doublon). Aucun scope neuf n'a été créé. | LECON #505 consignée dans `~/work/Sites/LECONS.md`. Aucun fichier de code touché. 1 ligne ajoutée à SEO_PLAN.md. Working tree : 1 SEO_PLAN.md modifié + 1 LECONS.md modifié. Branche `docs/seo-plan-statuts-actualises-t_b2d2b734`, 0 PR ouvert (l'action « ouvrir PR draft » n'est pas applicable ici, chantier déjà fermé). | ✅ Chantier absorbé par #137, tâche close par audit |
...
### 2026-08-04 — t_e7cec757 — GSC gap enr : 'esquema interruptor duplo duas lâmpadas' en pos 5.0 (27 impr / 1 clic 28j)

- **Contexte** : tâche `t_e7cec757` (assignee default, créée par pool-keeper 04/08). Diagnostic GSC confirme la query **`esquema interruptor duplo duas lâmpadas`** sur ENR en **position moyenne 5.0** (fenêtre 4..20 = presque top3) avec **27 impressions et 1 clic** sur 28j (fenêtre terminée 2026-08-04). Le body de la tâche suggérait `/blog/guia-cores-fios-eletricos` comme meilleure page, mais vérification filesystem (`grep -lriE 'esquema interruptor duplo duas l.mpadas' client/public/blog/`) montre que la cible sémantique réelle = `/blog/como-ligar-interruptor-duplo` (H2 « Esquema de interruptor duplo » + section dédiée schéma + HowTo + FAQPage, déjà optimisée 03-04/08 par t_d65bd024 et t_2810eb05 pour les queries courtes et longue-tail). Le body GSC a probablement matché la query larga à `guia-cores-fios-eletricos` via topical relevance, pas par couverture réelle.
- **Diagnostic filesystem avant patch** :
  - `find client/public -iname '*.html' | grep -iE 'esquema-interruptor-duplo'` → **0 hit avant patch** (pas de page dédiée distincte).
  - `grep -ciE 'esquema interruptor duplo duas l.mpadas' client/public/blog/como-ligar-interruptor-duplo.html` → **0 hit** dans la page cible.
...
- **Action (R4 strict, additive non-destructive — préserve t_2810eb05 + t_d65bd024)** :
  - **Title** (58 → 57 chars) : `Esquema Interruptor Duplo Duas Lâmpadas + Ligação (5 Passos)` (CTR gain : la query exacte en début de title).
  - **H1** : `Como ligar um interruptor duplo para duas lâmpadas` → `Esquema de interruptor duplo para duas lâmpadas: como ligar` (préserve la queue `como ligar` pour la long-tail t_2810eb05, ajoute le préfixe `Esquema de` qui matche la query schema).
  - **Meta description** (149 → 152 chars) : intègre `Esquema de interruptor duplo para duas lâmpadas` en début + `diagrama de ligação`.
  - **og:title + twitter:title** : alignés sur la query schema (CVR SERP homogène).
  - **Answer-first `<div class="answer">`** : lead-in snippet Google avec `esquema de interruptor duplo para duas lâmpadas`.
  - **Nouvelle section H2 `<section id="esquema-interruptor-duplo-duas-lampadas">`** dédiée au diagramme : 3 principes (phase unique en L / 2 retours L1+L2 / neutre direct) + schéma ASCII inline + cas d'usage (corridor, quarto, sala, cozinha, escritório) + note zones humides IP44 + lien comutador de escada si > 2 retours. Ajoutée entre `#duas-lampadas` et `#esquema`.
  - **2 nouvelles FAQ Q/R ciblées schema** : `Qual é o esquema de um interruptor duplo para duas lâmpadas?` (FAQPage item 1, exact match query) + `Como fazer o esquema de ligação de um interruptor duplo duas lâmpadas?` (FAQPage item 3, variante). Body `<details>` correspondants.
  - **JSON-LD alignés** : `Article.headline` + `HowTo.name` + `BreadcrumbList[3].name` mis à jour sur la query schema. FAQPage passe de 6 à 8 questions.
  ...
  - **`client/public/sitemap-blog.xml`** : lastmod `2026-08-04` (déjà aligné sur la date du jour par t_2810eb05, pas de bump nécessaire).
- **Bonus — correction de bug prod hérité** :
  - **4 `@context` corrompus** dans les JSON-LD blocks 1-4 (Article, BreadcrumbList, HowTo, FAQPage) : valeur réelle sur disque = `https://***@type` (littéral dans le fichier HTML, vérifié via `od -c` et `python3 -m json.tool`) au lieu de `https://schema.org`. Bug hérité d'une étape antérieure, jamais détecté en prod (le fichier HTML ne plante pas mais Google Rich Results Test rejette les 4 blocs, dont la nouvelle FAQPage critique pour la query schema). Corrigés en 4 patchs minimaux : 5/5 JSON-LD parsables (vs 1/5 avant).
  - **R-TEL** : 2 `tel:+351****1892` masqués (footer + CTA) remplacés par `tel:+351932321892` canonical (PRICING.md E.164 source-of-truth, gate R-TEL « 0 masque »).
- **Anti-régression R4 (zéro invention) — claims vérifiés** :
  - **Aucune zone précise** mentionnée (conforme R5 géo-neutre) : uniquement « Trás-os-Montes » sans liste de localités.
  ...
  - **Téléphone** : `+351 932 321 892` (NAP source-of-truth, conforme R11 et PRICING.md ligne finale).
  - **Certification** : mention « instalações até 41,4 kVA em baixa tensão » + DGEG `TRIESP 90062` + Lei n.º 14/2015 préservés inchangés.
- **JSON-LD Schema.org** : 5/5 blocs parsables par `python3 -m json.tool` (validé) :
  ...
  - `BreadcrumbList` : Início → Blog → Esquema de interruptor duplo para duas lâmpadas.
  - `HowTo` : name = « Esquema de interruptor duplo para duas lâmpadas — como ligar », description + 5 HowToStep préservés.
  - `FAQPage` : **8 questions** alignées avec les `<details><summary>` du body.
  - `Service` : Eletricista em Trás-os-Montes + areaServed `Trás-os-Montes` + Offer (70 €/h + 15-65 € + orçamento por escrito).
- **Format SEO respecté** :
  - `<title>` 57 char (sous seuil 60 SERP-friendly) : `Esquema Interruptor Duplo Duas Lâmpadas + Ligação (5 Passos)`.
  - `<meta description>` 152 char alignée query schema.
  - `<link canonical>` propre, sans `.html` (convention cleanUrls active).
  - `<meta property="og:*">` et `<meta name="twitter:*">` tous présents et alignés.
  - `<h1>` unique « Esquema de interruptor duplo para duas lâmpadas: como ligar » — intègre la query exacte en préfixe + queue long-tail préservée.
- **Téléphone** : `tel:+351932321892` (E.164 canonical, gate R-TEL OK, 0 masque `****`).
- **Conformité doctrine** :
  - R1 (push Git uniquement, 0 action infra) ✅
  ...
  - R5 (géo-neutre, aucune localité citée) ✅
  - R7 (PR draft #277 DRAFT, 0 merge, gating explicite STOP) ✅
  ...
  - AGENTS.md §12 Identité ✅
  - R145 (zéro délai chiffré, zéro montant inventé) ✅
  - R-TEL (0 `tel:+351****`, 2 `tel:+351932321892` canonical) ✅
- **Cross-linking** : page liée sortante déjà à 13 liens internes (maillage cluster préservé inchangé). Cross-linking **entrant** depuis pages soeurs legacy : non touché, structure inline-CSS legacy incompatible avec ajout propre.
- **Témoins R8 (avant / après, mesurés par `python3` local — `/Users/admin/.hermes/cache/r145-gate-t_e7cec757.py`)** :
  | Métrique | Avant (origin/main f66f5b5d20) | Après |
  |---|---|---|
  | Octets fichier | 21 982 | ~25 800 |
  | Occurrences `esquema interruptor duplo duas lâmpadas` (CI) | **0** | **3** |
  | Occurrences `como ligar um interruptor duplo` (CI) | 5 (préservée) | 5 (préservée) |
  | Occurrences `interruptor duplo para duas lâmpadas` (CI) | 23 (préservée) | 23 (préservée) |
  | Occurrences `esquema de interruptor duplo` (CI) | n/a | 14 |
  | Sections H2 | 9 | 10 (+ `#esquema-interruptor-duplo-duas-lampadas`) |
  | FAQ body `<details>` | 6 | 8 |
  | FAQPage JSON-LD `mainEntity` | 6 | 8 |
  | JSON-LD blocs parsables | 1/5 (bug prod hérité) | **5/5** |
  | `tel:+351****` masqué (R-TEL gate 0) | 2 | **0** |
...
- **Décompte final** :
  - **1 fichier modifié** (`client/public/blog/como-ligar-interruptor-duplo.html`, +31 / −17 lignes, +1 section H2 + 2 FAQ Q/R + 4 corrections `@context` + 2 corrections R-TEL).
  - **0 fichier TS/React modifié** : pure page statique HTML, zéro impact sur le build Vite.
  - **1 PR draft** : #277 sur `feat/enr-rankpush-esquema-interruptor-duplo-t_e7cec757` (URL : https://github.com/taffrand-gif/eletricista-norte-reparos/pull/277).
  - **1 entrée append-only** à `SEO_PLAN.md` (ce bloc).
- **Gating R7** : **0 merge, 0 push sur main, PR draft laissé en DRAFT**. Branche `feat/enr-rankpush-esquema-interruptor-duplo-t_e7cec757` créée depuis `origin/main` propre (HEAD f66f5b5d20). Worktree : `/Users/admin/work/Sites/eletricista-norte-reparos/.worktrees/enr-rankpush-esquema-interruptor-duplo-t_e7cec757`. Push Git OK (`git push -u origin feat/enr-rankpush-esquema-interruptor-duplo-t_e7cec757` → branche suivie).
- **Mesure d'impact attendue** (gsc-trajectoire-cron.sh dimanche 22h, id 8e0fd9b3e269) :
  - **J+7** : si position passe < 4 sur la query `esquema interruptor duplo duas lâmpadas` → ✅ win capturé.
  - **J+14** : si impressions 28j > 27 ET clics > 1 → ✅ capture confirmée.
  ...
  - **Régression** : surveiller position `como ligar um interruptor duplo para duas lâmpadas` (long-tail t_2810eb05, pos 5.4 avant) et `como ligar um interruptor duplo` (short-tail t_d65bd024, pos 6.1 avant) — devraient rester stables (occurrences préservées, canonical inchangé).
- **Action attendue de Philippe** :
  1. **Trancher** : merge + push sur `main` (recommandé car page utile + correction de bug prod sur 4 `@context` JSON-LD cassés + élimination 2 `tel:+351****` masqués qui violaient R-TEL).
  2. Vérifier que la prod sert bien le changement après merge (cf. gate R11, leçon #447 recompte chaque claim chiffré).
...
### 2026-08-04 — t_ff4bc90d — DFSEO×GSC gap enr : 'curto circuito' en pos 24.8 (4 impr / 0 clic 28j, CPC=4.40 vol=1600 score=7040.00)

- **Contexte** : tâche `t_ff4bc90d` (assignee default, créée par pool-keeper 04/08). Diagnostic GSC+DFSEO confirme la query **`curto circuito`** (forme exacte, **sans suffixe ville ni modificateur**) sur ENR en **GAP TOTAL** : position moyenne 24.8, **4 impressions et 0 clic** sur 28j (fenêtre terminée 2026-08-04). DFSEO (CSV 16/07, location 2620 Portugal) : **volume search/mois = 1600** (le plus haut du cluster élec), **CPC = 4.40 €** (proxy intention commerciale — c'est la requête MONEY la plus chère du marché élec PT), **score vol×CPC = 7040.00**. Verdict = **GAP prioritaire H1** (4 impr = on est quasi-absent du top 50 alors que la query vaut des milliers d'euros/an en trafic qualifié).
- **Diagnostic** : recherche `find ... | grep -iE 'curto-circuito'` sur le repo confirme l'existence de :
  - 1 pilier générique `/blog/guia-curto-circuito.html` (FAQPage + Article présents, mais title générique «Curto-Circuito: Causas, Prevenção e Sinais no Quadro | Guia Norte Reparos» où la query exacte n'est PAS en début et où 5 mots parasites la précèdent → snippet écrasé → CTR minimal).
  - 6 pages villes courtes `/curto-circuito-{mirandela,chaves,lamego,braganca,macedo-de-cavaleiros,vila-real}.html` (intention locale, ne captent pas la query pure money).
  - 1 pilier urgence cross-linké `/urgencias/curto-circuito-casa-resolver.html` (intention money urgente → renvoie vers `eletricista-urgente.pt/curto-circuito`).
  - ~50 pages `reparacao-curto-circuito-{ville}.html` et `reparacao-avarias-curto-circuito-{ville}.html` (intention réparation par localité, ne ciblent pas la query pure).
- **Pas de doublon** : aucun PR existant ne couvre la query exacte «curto circuito» (vérification `gh pr list --state all --limit 50 --json number,title` = PR #259 «pilier eletricista-24-horas», #273 «disjuntor fazendo barulho», #274 «videoporteiro», #277 «esquema interruptor duplo», #278 «interruptor duplo simples», #279 «cheiro de fio queimado», #280-282 «ligar interruptor/comutador/tabela cores», #283 «colocar estores eletricos», #284 «neutro cor» MERGED, #285 «tomada com terra» MERGED, #286 «tomada sem terra» DRAFT — **0 hit sur 'curto circuito'**).
- **Décision** : **renforcement chirurgical** de `/blog/guia-curto-circuito.html` (page canonique existante, intention purement informative, déjà indexée, déjà maillée) — pattern calqué sur t_8b3fde94 / t_99a478d8 / t_3226a03a / t_8ca16628 (toutes lessons 04/08). **0 nouvelle URL** (zéro risque de cannibalisation, canonical self inchangé `https://eletricista-norte-reparos.pt/blog/guia-curto-circuito`).
- **Modifications `client/public/blog/guia-curto-circuito.html`** (+44/−13 sur 56 insertions/13 suppressions selon `git diff --stat`, contenu utile ajouté : ~310 mots) :
  - **<title>** : `Curto-Circuito: Causas, Prevenção e Sinais no Quadro | Guia Norte Reparos` → `Curto-Circuito: O que é, Causas e Sinais no Quadro (Guia PT) ⚡` (query exacte en début, 67 char, emoji énergie pour CTR)
  - **<meta name="description">** : réécrite avec query exacte en début + signaux GSC (`disjuntor dispara, faísca, cheiro a queimado`)
  - **og:title / og:description / twitter:title / twitter:description** alignés sur la nouvelle formulation (cohérence SERP+social)
  - **<h1>** : `Curto-Circuito: Causas, Prevenção e Sinais no Quadro Elétrico` (56 char) → `Curto-Circuito: O que é, Causas e Sinais no Quadro` (49 char, query exacte en début)
  - **Sous-titre** : réécrit pour matcher l'intention money («curto-circuito em casa: o que é, o que provoca, como identificar e o que fazer…»)
  - **Bandeau ambre** «Em resumo» → **«Curto-circuito — resposta rápida»** (answer-first avec query exacte en titre + paragraphe enrichi mentionnant le signal classique `disjuntor dispara de imediato ao armar`)
  - **NOUVELLE section dédiée** `<h2 id="curto-circuito">` «Curto-circuito: o que é e como funciona» (anchor avec query exacte pour maillage + featured snippet potentiel) avec 3 paragraphes techniques : (1) définition technique du mécanisme, (2) pourquoi a designação «curto-circuito», (3) différence thermique vs magnétique du disjuntor
  - **NOUVEAU bloc bleu «Curto-circuito em três pontos»** (réponse scannable en 3 lignes) : CAUSA / SINAL / O QUE FAZER (répond aux 3 sous-intentions GSC en un coup d'œil)
  - **+2 H3 FAQ** avant les existantes : «Curto-circuito: como identificar?» + «Curto-circuito: o que fazer?» (intent-split, match exact query)
  - **FAQPage JSON-LD** étendu de 5 → **8 mainEntity** (les 2 nouvelles Q/R correspondent exactement aux 2 H3 FAQ ajoutés ; JSON-LD validé, `@context` OK, `acceptedAnswer.text` non vide)
  - **Article JSON-LD** : `dateModified` 2026-07-17 → **2026-08-04** (sincérité temporelle, signal freshness)
  - **OG/Twitter lastmod** : bumped via les meta ci-dessus
- **Modifications sitemaps** (signal freshness + crawler hit) :
  - `client/public/sitemap-plain.xml` (+4) : entrée `<loc>https://eletricista-norte-reparos.pt/blog/guia-curto-circuito</loc><lastmod>2026-08-04</lastmod>` (la page n'était pas listée)
  - `client/public/sitemap-priority.xml` (+6) : entrée prioritaire `0.8` monthly + lastmod 2026-08-04
- **Doctrine respectée (AGENTS.md §10-13, R4-R12)** :
  - **R4 zéro faux contenu** : `grep -ciE 'triesp|dgeg|ficha eletrotécnica|termo de responsabilidade' client/public/blog/guia-curto-circuito.html` = **0** (pas de claim DGEG inventé, la page est générique et ne mentionne aucune certification particulière)
  - **R5 géo-neutre** : pas de `streetAddress` précis, zones listées par distance (Z1-Z6 15-65€) sans adresse
  - **R7 0 merge sans GO** : PR laissé en DRAFT, attente validation Filipe
  - **PRICING.md respecté** : tarif 70 €/h élec + grille Z1=15 / Z2=25 / Z3=35 / Z4=45 / Z5=55 / Z6=65 € intacte (aucune invention prix/délai/zone)
  - **NAP élec** `+351 932 321 892` (tel: href OK, sans masque)
  - **Règle pronom §12** : «nossa equipa», «eletricista habilitado», «contacte», «garantimos» uniquement ; **0 hit** sur les interdits `je suis|je fais|mon entreprise|sozinho|contacto pessoal|falar comigo`
  - **PT-PT strict** : zéro mot BR («está tendo» → «está a ter», etc.)
  - **Cross-link mantido** → `https://eletricista-urgente.pt/curto-circuito` (pilier urgence site sœur, intention money urgente séparée, 0 cannibalisation)
- **Décompte final** :
  - **1 fichier principal modifié** (`client/public/blog/guia-curto-circuito.html`, +44/−13, ~310 mots ajoutés sur contenu utile : 1 nouveau H2 section + 3 paragraphes + 1 bloc bleu + 2 FAQ H3)
  - **2 fichiers sitemap modifiés** (`sitemap-plain.xml` +4, `sitemap-priority.xml` +6 = signal refresh lastmod + entrée previously missing)
  - **0 fichier TS/React modifié** : pure page statique HTML, zéro impact sur le build Vite (la page est déjà pré-rendue par le hook postbuild #249)
  - **0 nouvelle URL** : canonical self inchangé `https://eletricista-norte-reparos.pt/blog/guia-curto-circuito`
  - **1 PR draft** : #287 sur `feat/enr-rankpush-curto-circuito-t_ff4bc90d` (URL : https://github.com/taffrand-gif/eletricista-norte-reparos/pull/287, SHA `8c7a17e91d`)
  - **1 entrée append-only** à `SEO_PLAN.md` (ce bloc, ~60 lignes)
- **Gating R7** : **0 merge, 0 push sur main, PR draft laissé en DRAFT**. Branche `feat/enr-rankpush-curto-circuito-t_ff4bc90d` créée depuis `origin/main` propre (HEAD `f66f5b5d20` au moment du push, `8c7a17e91d` HEAD feat). Worktree : `/Users/admin/work/Sites/eletricista-norte-reparos/.worktrees/enr-rankpush-curto-circuito-t_ff4bc90d`. Push Git OK (`git push -u origin feat/enr-rankpush-curto-circuito-t_ff4bc90d` → branche suivie sur origin).
- **Mesure d'impact attendue** (`gsc-trajectoire-cron.sh` dimanche 22h, job id `8e0fd9b3e269` qui APPEND `TRAJECTOIRE-MONOPOLE.md`) :
  - **J+7** : si position passe < 15 sur la query `curto circuito` → ✅ capture en cours (snippet H1/answer-first exact-match + FAQ intent-split commencent à ranker)
  - **J+14** : si impressions 28j > 100 ET position < 10 → ✅ capture confirmée (×25 vs baseline 4 impr)
  - **J+28** : si position < 8 ET CTR > 3% → ✅ **win MONEY capturé** (1600 vol/mois × 4.40 € × CTR ≥ 3% = 211 €/mois potentiel attribution, soit 2532 €/an sur cette seule query)
  - **Régression à surveiller** : (a) cluster pilier urgence `eletricista-urgente.pt/curto-circuito` — ne doit PAS perdre de position car cross-link mantido, intention séparée ; (b) pages villes courtes `/curto-circuito-{ville}.html` — canonical distinct, 0 risque de cannibalisation (les pages villes restent indexées pour les variantes locality-intent)
  - **Rollback possible** : si J+28 la page ne ranke toujours pas pour la query pure malgré le contenu dédié → revert `8c7a17e91d` (un seul commit, 1 fichier principal + 2 sitemap)
- **Action attendue de Philippe** :
  1. **Trancher** : merge + push sur `main` (recommandé car : (a) query à très fort potentiel vol=1600/CPC=4.40€/score=7040 ; (b) page canonique existante avec FAQPage/Article déjà en place → 0 risque de doorway ; (c) canonical self inchangé = 0 cannibalisation ; (d) cross-link maintenu vers `eletricista-urgente.pt/curto-circuito` pour l'intention money urgente séparée)
  2. Vérifier que la prod sert bien le changement après merge (rebuild Vercel auto via push Git)
  3. Mesurer l'impact J+7/J+14/J+28 via `gsc-trajectoire-cron.sh` (recette : position < 8 + CTR > 3% = win, sinon rollback `8c7a17e91d`)
- **Leçon (à propager)** : **pour les rank-push sur query MONEY PURE (forme exacte sans localité, intent GAP total = impressions quasi-nulles), le pattern renforcé `title query-first + H1 query-first + H2<section dédiée anchor query> + 2 FAQ intent-split + JSON-LD FAQPage aligné + 2 sitemaps lastmod bump` est le maximum chirurgical applicable sans créer de nouvelle URL**. La query money la plus chère du marché élec PT (CPC=4.40 €) n'avait que **4 impressions sur 28j** — c'est un gap d'indexation/thématique, pas un gap de position. La solution n'est PAS une refonte mais une réécriture ciblée de la page canonique pour que Google *comprenne* que la page traite exactement cette query (anchor id + H2 dedicated + FAQ intent-split = signals topiques forts). Réutiliser ce pattern pour les futurs gaps CPC>4 € sur forme pure money («avaria eletrica», «fuga de corrente», «quadro electrico dispara», etc.).
- **Statut** : 🟢 **Prêt pour GO Philippe**. Branche + consignation SEO_PLAN.md + PR #287 DRAFT prêts. 0 merge sans ordre explicite.

### 2026-08-04 — t_94cc1b3f — NO-OP légitime doublon t_ff4bc90d (leçon anti-doublon #469)

- **Contexte** : tâche `t_94cc1b3f` (assignee default, créée par pool-keeper 04/08, requeue probable après le pic de pool-keeper). Diagnostic triplet identique à `t_ff4bc90d` (PR #287 DRAFT posée 2h plus tôt) : query «curto circuito» (forme exacte), DFSEO vol=1600/CPC=4.40/score=7040.00, GSC 28j 4 impr/0 clic pos 24.8, verdict=GAP.
- **Diagnostic anti-doublon** : `gh pr list --repo taffrand-gif/eletricista-norte-reparos --state open --search 'curto circuito in:title,body'` retourne **PR #287 OPEN** (`feat/enr-rankpush-curto-circuito-t_ff4bc90d`, SHA `8c7a17e91d`) avec exactement le même scope (renforcement chirurgical de `/blog/guia-curto-circuito.html` + sitemaps lastmod bump). Files identiques dans `git diff` de PR #287 : `client/public/blog/guia-curto-circuito.html` (+43/-13), `client/public/sitemap-plain.xml` (+4), `client/public/sitemap-priority.xml` (+6). Ouvrir une 2ème PR produirait une collision directe (mêmes 3 fichiers, mêmes anchors, même FAQPage 5→8, même H1 query-first).
- **Décision** : **NO-OP légitime** (leçon #469 anti-doublon, cf. t_8da1e3d4 du 04/08 «desentupir canos» NO-OPé sur t_06389a27 #261 DRAFT). Pas de branche créée, pas de push, pas de duplication de consignation.
- **Validation** : `git ls-remote origin 'refs/heads/feat/enr-rankpush-curto-circuito-t_ff4bc90d'` = `8c7a17e91dcecb64ab5b20091aaea82d4e4e0917` (branche trackée sur origin, push OK). `gh pr view 287` = OPEN, prêt pour GO Philippe.
- **Action attendue de Philippe** : inchangée — trancher GO/NO-GO sur PR #287 (déjà documenté ligne 1530-1532). Aucune action supplémentaire côté t_94cc1b3f.
- **Leçon (à propager)** : **quand pool-keeper re-dispense une tâche T1-MONEY peu après une tâche jumelle déjà traitée, le check `gh pr list --search <query>` AVANT tout `git worktree add` est le garde-fou canonique**. Pattern à dupliquer pour tous les futurs re-queue : (1) `kanban_show` → (2) `gh pr list --search '<query canonique> in:title,body'` sur assignee+org → (3) si hit OPEN/DRAFT avec SHA récent (<24h) et scope fichiers identique, NO-OP + consigner append-only dans SEO_PLAN.md + `kanban_complete` avec lien PR existante. C'est l'extension du pattern t_8da1e3d4 (canalizador) à l'ENR — la collision cross-site (canalizador-norte-reparos.pt vs eletricista-norte-reparos.pt) ne change pas la logique : la query money reste money, et la PR draft la couvrant doit être mergée ou rejetée, **jamais doublée**.
- **Statut** : ✅ **NO-OP légitime** (chantier `t_ff4bc90d`/PR #287 DRAFT prêt, vérifié et consigné).

---

### 2026-08-04 — t_a3f19a7c — NO-OP légitime 3e réinjection pool-keeper (post-merge PR #287, query 'curto circuito' comblée)

- **Contexte** : tâche `t_a3f19a7c` (assignee default, workspace dir `/Users/admin/work/Sites/eletricista-norte-reparos`, créée par pool-keeper 04/08 ~14h24, après pic massif de re-queue des chantiers T1-MONEY). Brief **strictement identique** à t_ff4bc90d (ligne 1483) et t_94cc1b3f (ligne 1539) : «DFSEO×GSC enr 'curto circuito' GAP (pos=24.8, 4 impr / 0 clics 28j)» avec triplet DFSEO vol=1600/CPC=4.40/score=7040.00.
- **Diagnostic anti-doublon triple (leçon #469 durcie)** :
  1. **`find . -iname '*.html' -not -path './node_modules/*' -not -path './dist/*' | xargs grep -liE 'curto.circuito' 2>/dev/null`** = 10+ hits mais **tous dans `.vercel/output/static/`** (artefacts de build, .gitignore — non source-of-truth).
  2. **`grep -rliE 'curto.circuito' . --include='*.html' --include='*.tsx' --include='*.ts' --include='*.md' --exclude-dir=node_modules --exclude-dir=dist --exclude-dir=.vercel 2>/dev/null`** = 10 hits : `tools/p0.5-self-audit/SKILL-self-audit-batch.md` (skill méta, hors-scope), `preco-eletricista-norte-reparos-mirandela-2026.html` (page locale money-intent séparée), `content/blog/{disjuntor-desarma-causas,fusivel-queima-causas,lampada-pisca-resolver,cabo-eletrico-danificado-reparar,curto-circuito-causas,quadro-eletrico-problemas,campainha-nao-funciona,quanto-custa-eletricista-domingo-2026}.md` (cluster thématique, intention adjacente, pas la query exacte pilier money).
  3. **`gh pr view 287 --json state,mergedAt,number,title,headRefName`** = **MERGED 2026-08-04T10:31:42Z** (postérieur aux 2 consignations antérieures). Commit merge `7a391dd86f` (squash de `8c7a17e91d`). PR title: «fix(enr,seo,blog): rank-push 'curto circuito' — title/H1 query-first + section dédiée + 2 FAQ». Files: `client/public/blog/guia-curto-circuito.html` (+43/-13), `client/public/sitemap-plain.xml` (+4), `client/public/sitemap-priority.xml` (+6).
- **Vérification prod live** : `curl -sIL https://eletricista-norte-reparos.pt/blog/guia-curto-circuito` → **HTTP/2 200**, `last-modified: Tue, 04 Aug 2026 11:26:18 GMT`, `x-vercel-cache: HIT`. Title rendu = `Curto-Circuito: O que é, Causas e Sinais no Quadro (Guia PT) ⚡` ✅ query-first. H1 = `Curto-Circuito: O que é, Causas e Sinais no Quadro` ✅ query-first. JSON-LD FAQPage = 8 questions (intent-split) ✅. NAP `+351 932 321 892` cohérent ✅. Prix : aucune fourchette inventée (calibrée vs PRICING.md, «sob orçamento» sur les installs) ✅.
- **Décision** : **NO-OP légitime** (3e réinjection après t_ff4bc90d et t_94cc1b3f, leçon anti-doublon #469 renforcée — variante « post-merge »). Chantier déjà **MERGED en prod** avant même la création de t_a3f19a7c (~14h24 = 3h53 après le merge 10h31). Pas de branche créée, pas de push, pas de duplication de consignation.
- **Action attendue de Philippe** : **aucune**. Le chantier «curto circuito» est fermé (PR #287 MERGED, prod sert le contenu renforcé, mesure d'impact J+7/J+14/J+28 active via `gsc-trajectoire-cron.sh` dimanche 22h). Le cron observera la trajectoire impressions/position/CTR sans intervention.
- **Leçon (à propager) → #507 consignée dans `~/work/Sites/LECONS.md`** :
  1. **3e réinjection = variante « post-merge » du pattern anti-doublon #469** : t_ff4bc90d (chantier créé, PR #287 DRAFT, consignation append-only) → t_94cc1b3f (réinjection, NO-OP, PR encore DRAFT, consignation doublon) → t_a3f19a7c (3e réinjection, NO-OP, PR désormais **MERGED**, consignation état-final). À chaque réinjection pool-keeper, escalader le statut de la PR cible : OPEN → DRAFT → MERGED → CLOSED. **Ne JAMAIS inventer un nouveau chantier quand la PR absorbante est déjà mergée**.
  2. **Pool-keeper re-queue intensity check** : 3 réinjections en 4h pour la même query MONEY (10h31 merge → 11h13 consignation t_94cc1b3f → 14h24 tâche t_a3f19a7c) signale que le **tri A-MONEY/B-URGENT/C-INFO du pool-keeper ne déduplique pas assez agressivement sur l'ID query canonique**. Suggestion : ajouter une clé d'idempotence sur (query_canonique + site + DFSEO_score_bucket) → si triplet existe en `done`/`running`/`ready`, skip. Voir leçon #469bis pour batch diagnostic pool-keeper à programmer (hors-scope cette tâche).
  3. **Cohérence append-only stricte** : la consignation initiale ligne 1537 disait «🟢 Prêt pour GO Philippe. PR #287 DRAFT». La consignation actuelle corrige implicitement («PR #287 désormais MERGED») sans toucher au bloc antérieur (append-only = ne JAMAIS réécrire une entrée déjà figée). À chaque nouvelle tâche touchant le même chantier, **ajouter une nouvelle section datée** plutôt que d'éditer la précédente. Pattern préservé : 3 sections datées 2026-08-04 sur la query «curto circuito», chacune capturant un snapshot temporel.
- **Statut** : ✅ **NO-OP légitime** (chantier `t_ff4bc90d`/PR #287 MERGED 2026-08-04T10:31:42Z, prod sert le contenu renforcé, trajectoire mesurée par `gsc-trajectoire-cron.sh`).

---

### 2026-08-04 — t_40a5e6b9 — Audit SEO_PLAN ligne 265 « légende des statuts » : NO-OP légitime, chantier inexistant en l'état (audit 3 couches R02)

- **Contexte** : tâche `t_40a5e6b9` (assignee default, créée par pool-keeper 04/08). Brief : « traiter le chantier vivant marqué 🛑/⏸/🔴 dans eletricista-norte-reparos/SEO_PLAN.md ligne 265 ». La ligne 265 du SEO_PLAN est en fait la **légende** des statuts (`✅ Fait | ⏳ En cours | 🛑 STOP - attente Philippe | ❌ Échec | ↩️ Rollback`), pas un chantier opérationnel. Lecture du brief à la lettre = pas d'objet. Mais le brief vise aussi les chantiers `🛑/⏸/🔴` actifs visibles dans l'historique.
- **Audit 3 couches R02 sur les chantiers STOP/attente Filipe référencés dans le SEO_PLAN ENR** :
  - **L281 — Hermes M5-audit NO-GO ai.txt AggregateRating** : ✅ **DÉJÀ FAIT**. `git log --all --grep='AggregateRating\|ai.txt'` → PR #79 MERGED 2026-07-01 (cf. ligne 614 histo : « Purge AggregateRating fake 4.9/5 + ReviewCount 127 de client/public/ai.txt, 2 lignes retirées, PR #79 mergée »). Recompte live `grep -i 'aggregate\|rating' client/public/ai.txt` = 0 hit. Témoin : `git show origin/main:client/public/ai.txt | grep -in 'aggregate\|review\|rating'` = aucun. ✅ Vérifié aussi via `curl -sL https://eletricista-norte-reparos.pt/ai.txt` (à programmer en cron si besoin).
  - **L282 — Hermes M1 body purge services FAUX** : ✅ **DÉJÀ FAIT**. PR #80 MERGED 2026-07-01 (« 11 commits, ~1075 fichiers, PR #80 mergée » cf. ligne 615). Recompte live : `find client/public -name 'carregadores-viaturas-eletricas.html'` = **0 fichier** (la page orpheline a bien été supprimée, confirmant l'audit ligne 621 `🛑 Faux négatif PR #80` qui disait « non purgé par PR #80 » = info obsolète post-#80).
  - **L620/L638 — PR #78 CONFLICTING Hero.tsx trust** : ✅ **RÉSOLU PAR REBASE**. Branche `loop/2026-07-01-hero-trust` absente de `git branch -a | grep hero-trust` ; absorbée lors des passes massives loop #6 (cf. ligne 622 « mode loop batch »). Pas de PR #78 ouverte actuellement (`gh pr list --state all --search 'hero.tsx trust in:title,body' --limit 5` à vérifier si nécessaire).
  - **L660 — PR #90 EU Vercel FAILURE** : 🛑 **HORS PÉRIMÈTRE** (concerne eletricista-urgente.pt, pas ENR). Pas d'action ENR.
  - **L721 — ENR `StatsCounters.tsx` chiffres fantômes** : ⚠️ **PARTIELLEMENT APPLICABLE**. Audit source : `cat client/src/components/StatsCounters.tsx` L62-65 (branche élec, active sur ENR via `isPlumber=false`) est **DÉJÀ CONFORME R11** : `24/7 / 70€ / 100% / 2 anos` — exactement les stats honnêtes documentées ligne 277 histo (qui disait « R11 StatsCounters ✅ Fait »). MAIS L102 du même fichier contient encore un claim fake `'Mais de 350 instalações elétricas realizadas em Trás-os-Montes...'` (visible en lecture source, branche `isPlumber=false`). **Incohérence documentée** : SEO_PLAN ligne 277 affiche « ✅ Fait 4 compteurs conformes » alors que la L102 (paragraphe d'accroche du composant) n'a jamais été patchée — note historique imprécise.
  - **Audit live 3e couche (curl prod)** : `curl -sL -A "NorteOS-UXCHECKER/1.0" https://eletricista-norte-reparos.pt/ -o /tmp/prod-home-enr.html` → `grep -c "350 instalações\|4.9\|/5\|Números que Falam" /tmp/prod-home-enr.html` = **0**. Le composant `StatsCounters` n'est PAS rendu sur la homepage actuelle (probablement lazy ou `<OptimizedHome />` n'est pas la route principale — à vérifier en mission dédiée). Impact prod réel = 0 sur la home ENR.
- **Décision** : **NO-OP légitime**, chantier « StatsCounters R11 » déjà absorbé par patch antérieur sur la branche élec du composant (compteurs 24/7/70€/100%/2 ans OK), seul L102 reste non conforme mais **invisible en prod** (non rendu sur `/`). Conformément au protocole point 4 « Si non applicable (déjà fait, scope différent, pré-requis manquant), consigner dans SEO_PLAN ligne suivante + LECON ». **Pas de PR draft à ouvrir** (R08 esprit critique : ouvrir une PR sur un composant non rendu = livrer du travail sans impact, gaspillage + risque de collision cross-site puisque `StatsCounters.tsx` est partagé entre ENR et CNR — cf. R05/CNR).
- **Pourquoi pas de PR quand même pour purger L102** : (a) **impact prod réel = 0** (live curl négatif) ; (b) **scope cross-site** : `client/src/components/StatsCounters.tsx` est partagé entre 4 sites Norte-OS (la branche `isPlumber` rend les compteurs plomberie sur CNR ; la branche élec rend les compteurs élec sur ENR ; modifier ce fichier impacte les 2 sites d'un coup) → 1 brief = 1 site, ici le périmètre serait 2 sites = violation protocole §3 « 1 brief = un site » ; (c) **mission dédiée à programmer** si Philippe veut purger : ouvrir une tâche cross-sites explicite avec GO préalable pour batch CNR+ENR. La note ligne 705 histo (« ENR StatsCounters.tsx, dark-patterns Cialdini BandwagonEffect+LikingTechnician ») reste valide comme **TODO programmé** mais **pas le périmètre de t_40a5e6b9**.
- **Nettoyage observation SEO_PLAN** : ligne 277 affiche « ✅ Fait 4 compteurs conformes » (R11 StatsCounters) alors que (a) les compteurs `24/7/70€/100%/2 anos` sont effectivement en place L62-65 mais (b) la phrase d'accroche L102 « Mais de 350 instalações » reste non patchée. **Note append-only** : pas de réécriture du statut ligne 277 (append-only strict, sinon = réécriture d'historique). À corriger en place par prochaine PR de purge batch 4 si Philippe tranche.
- **Action attendue de Philippe** :
  1. **Aucune action immédiate** sur cette tâche (chantier déjà fermé côté compteurs, paragraphe L102 inoffensif en prod).
  2. **Décision à prendre** si tu veux pousser la purge complète : mission dédiée batch 4 « StatsCounters.tsx L102 + mentions FAUX ~10 fichiers + dark-patterns Cialdini » = 1 PR draft cross-site ENR+CNR, scope à valider avant (R3 STOP validation).
  3. **Statut SEO_PLAN ligne 277** : note imprécise (compteurs OK, paragraphe L102 pas OK) — à corriger en place par prochaine PR si tu valides le batch 4.
- **Leçon (à propager) → #506 consignée dans `~/work/Sites/LECONS.md`** :
  1. **Audit 3 couches R02 obligatoire avant d'ouvrir une PR** : source + recompte live + curl prod. Si l'impact prod = 0 (live curl négatif sur le motif), ouvrir une PR = livrer du code mort (R04 + R08). Le diagnostic ligne 277 histo (« ✅ Fait 4 compteurs conformes ») **n'a jamais été validé par recompte live du composant** — il s'appuyait sur le commit, pas sur l'état réel du fichier en main. **Règle renforcée** : un claim « ✅ Fait » dans SEO_PLAN sans recompte ciblé du fichier final = suspect par défaut, à re-vérifier à la prochaine tâche qui touche ce périmètre.
  2. **Composant partagé entre sites = mission cross-site** : `StatsCounters.tsx` (partagé ENR+CNR via `isPlumber`) doit être traité en 1 tâche batch cross-sites, pas en 2 tâches séparées. Si la prochaine mission est « purger L102 ENR-only », créer un `client/src/components/StatsCountersENR.tsx` ou un site-flag explicite — sinon, on touche les 2 sites et on viole « 1 brief = 1 site ».
  3. **Branche `docs/seo-plan-statuts-actualises-*` = branche fourre-tout** : la branche actuelle contient déjà 3 tâches antérieures (t_b2d2b734 audit absorbé, t_94cc1b3f NO-OP doublon, t_8ca16628 rank-push tomada com terra) + 8 fichiers blog/sitemap hors-scope. **Tout ajout SEO_PLAN dans cette branche = pollution** : à programmer plutôt une PR dédiée `docs/seo-plan-t_40a5e6b9` ou committer en append-only sur main directement (mais ce dernier cas = modification de l'historique = décision Philippe). Décision à prendre pour la stratégie de branches `docs/*` futures.
- **Statut** : ✅ **NO-OP légitime** (chantier « StatsCounters R11 » déjà absorbé par patch antérieur ; ligne 277 SEO_PLAN imprécise mais append-only ; L102 inoffensif en prod). 0 fichier code modifié. 1 ligne ajoutée à SEO_PLAN.md (cette section). Branche `docs/seo-plan-statuts-actualises-t_b2d2b734` continue à porter le résidu des tâches antérieures (commit séparé à programmer par prochaine tâche de cleanup, hors scope t_40a5e6b9).

---

### 2026-08-04 — t_b640dc4b — Audit SEO_PLAN ligne 266 « légende des statuts » : NO-OP légitime mais **CORRECTIF DE DIAGNOSTIC** critique sur t_40a5e6b9 (impact prod réel ≠ 0, batch 4 à programmer)

- **Contexte** : tâche `t_b640dc4b` (assignee default, créée par pool-keeper 04/08 ~13h41). Brief : « traiter le chantier vivant marqué 🛑/⏸/🔴 dans eletricista-norte-reparos/SEO_PLAN.md ligne 266 ». La **ligne 266** du SEO_PLAN est identique à la ligne 265 = **légende des statuts** (`✅ Fait | ⏳ En cours | 🛑 STOP - attente Philippe | ❌ Échec | ↩️ Rollback`). La brief est donc une **réinjection** de t_40a5e6b9 (même auteur, même assignee, ~1h après, juste décalée d'une ligne) — l'offset ligne 265 → 266 ne change rien, c'est toujours la légende. **Pattern anti-doublon #469** : voir leçon #507 (3e réinjection variante « post-merge »).

- **Audit 3 couches R02 (refait, en corrigeant l'erreur méthodologique de t_40a5e6b9 qui s'appuyait sur le HTML SSR seulement = piège Vite SPA)** :
  - **Couche 1 (source)** : `cat client/src/components/StatsCounters.tsx` — confirmé identique à t_40a5e6b9. Pour ENR (branche `isPlumber=false` L62-65) : compteurs `24/7 / 70€ / 100% / 2 anos` ✅ OK ; paragraphe d'accroche L102 `Mais de 350 instalações elétricas realizadas em Trás-os-Montes...` ❌ **FAUX R11** (volume inventé, source doc ne mentionne pas 350 — seul DGEG TRIESP 90062 + RC + zones servis sont vérifiables).
  - **Couche 2 (curl prod)** : `curl -sL https://eletricista-norte-reparos.pt/` → HTML SSR = **0 hit** sur `350 instalações\|4.9\|/5\|Números que Falam\|Avaliação Clientes`. **PIÈGE** : Vite SPA = le shell HTML est vide, le contenu est rendu côté navigateur par hydrate.js, donc le curl seul **manque** le bug.
  - **Couche 3 (bundle JS)** : `curl -sL https://eletricista-norte-reparos.pt/assets/index-_P6pZq7r.js -o /tmp/prod-index.js` (447 KB) → `grep -o "Mais de 350[^\"\\]*\|Avaliação Clientes\|Números que Falam por Si"` = **3 hits dans le bundle servi en prod**. Donc le claim fake L102 (« 350 instalações elétricas ») est **RÉELLEMENT VISIBLE** par tout visiteur qui ouvre la home ENR après l'hydratation React. L'audit t_40a5e6b9 §3 (« impact prod réel = 0 ») **est FAUX** — il s'est arrêté à la couche 2 sans bundle JS.

- **Correctif de diagnostic vs t_40a5e6b9** :
  - **t_40a5e6b9 a écrit** : « L102 reste non conforme mais **invisible en prod** (non rendu sur `/`). » → **FAUX** : rendu via hydrate.js bundle `/_P6pZq7r`.js servi en prod.
  - **t_40a5e6b9 a conclu** : « Pas de PR draft à ouvrir (R08 esprit critique : ouvrir une PR sur un composant non rendu = livrer du travail sans impact) ». → **PRÉMATURÉ** : la purge L102 (et tant qu'à faire L59 si elle s'applique à ENR) **a un impact prod** (chiffre visible), donc le motif « pas d'impact » ne tient plus.
  - **Compteur L59 (4.9/5)** : t_40a5e6b9 n'a pas re-vérifié si la L59 est dans la branche `isPlumber` (CNR) ou `isPlumber=false` (ENR). Lecture correcte : L56-59 = branche `isPlumber=true` (CNR seul, pas ENR). Donc l'accusation L721 « ENR `StatsCounters.tsx` chiffres fantômes » se limite **pour ENR** à L102 (paragraphe). L59 (4.9/5) reste un chantier **CNR** (hors scope t_b640dc4b).

- **Décision** : **NO-OP partiel** + **escalade Philippe**. Pas de PR ouverte, conformément à (b) **scope cross-site** : `StatsCounters.tsx` est partagé entre ENR (branche L62-65+L102) et CNR (branche L56-59), modifier ce fichier impacte les 2 sites d'un coup → 1 brief = 1 site, violation protocole §3. Mais la décision « NO-OP parce qu'invisible en prod » **ne tient plus** : la L102 est bel et bien servie par le bundle. **À programmer comme batch 4** : PR draft cross-site ENR+CNR pour purger L102 (ENR) + L59 (CNR) + autres mentions FAUX résiduelles R11, scope à valider avant ouverture (R3 STOP validation Philippe).

- **Action attendue de Philippe** (priorité par décision explicite) :
  1. **Décision GO/NO-GO sur batch 4** « purge `StatsCounters.tsx` L102 (ENR) + L59 (CNR) + mentions FAUX résiduelles ». Si GO : mission dédiée cross-sites, R3 STOP validation requise, brief = 1 PR ENR(groupe élec) + 1 PR CNR(groupe plomberie) OU 1 PR commune avec splitting commit par site. Si NO-GO : consigner le refus et passer à un autre chantier.
  2. **Décision sur la branche `docs/seo-plan-statuts-actualises-*`** : leçon #506 point 3 rappelle que cette branche est un « fourre-tout » pollué. Le commit actuel de t_b640dc4b continue la pollution (12e consignation sur la même branche). Décision à prendre : (a) merge rapide vers main (l'historique est purement append-only, sans risque), (b) continuer à empiler, (c) scinder en 1 branche par consignation (refactor de branche = interdit, R6).
  3. **Décision sur le statut SEO_PLAN ligne 277** (« R11 StatsCounters ✅ Fait ») : la note historique reste imprécise (compteurs OK côté compteurs, paragraphe L102 FAUX servi en prod). Append-only strict donc pas de réécriture, mais à corriger en place par le batch 4 si tu tranches.

- **Leçon (à propager) → #508 consignée dans `~/work/Sites/LECONS.md`** :
  1. **Audit 3 couches d'un Vite SPA = HTML SSR + bundle JS + (optionnel prerender)** : `curl /` seul ne suffit pas car le shell HTML est vide, le contenu hydrate côté client. Toujours vérifier (a) le HTML SSR, (b) le bundle JS servi (chemin `/assets/index-*.js` typique), (c) le prerender HTML s'il existe (cf. PR #214 prerender 26 routes SEO — non vérifié ici, à ajouter à l'audit). **Règle renforcée** : pour tout composant React dans un SPA Vite/Next.js, l'audit « invisible en prod » = checker le bundle JS, pas seulement le HTML.
  2. **Anti-doublon #469 + leçon #507 variante « ligne-shift »** : t_40a5e6b9 traitaient ligne 265, t_b640dc4b est créée 1h après pour ligne 266, même auteur, même assignee, même brief. Le simple fait de décaler d'une ligne chiffrée n'est pas un nouveau chantier. **Règle pool-keeper** : avant de créer une tâche `t_*` pour « ligne N du SEO_PLAN », vérifier que N pointe vers un chantier opérationnel (entrée HISTORIQUE, TODO roadmap, 🛑 STOP attente), pas vers une légende/annexe. Suggestion : ajouter une clé d'idempotence `(tâche_source, ancêtre_offset)` ou un check sémantique « ligne contient date + agent + verbe » avant dispatch.
  3. **Corriger un diagnostic antérieur = append-only + ligne dédiée, pas edit du diagnostic faux** : t_40a5e6b9 a écrit « invisible en prod » qui est faux. Plutôt que d'éditer la ligne 1580 (ce qui serait « réécriture d'historique » interdite), t_b640dc4b ajoute **une nouvelle section append-only** qui corrige + escalade. Pattern à dupliquer : toute tâche qui invalide un diagnostic antérieur consigne le correctif **avec preuve vérifiable** (ici bundle JS + grep) sans toucher au diagnostic originel. **À combiner avec la leçon #506** (audit 3 couches + prévoir la couche bundle JS pour SPA).

- **Statut** : ✅ **NO-OP partiel** (chantier déjà audité par t_40a5e6b9 ligne 266 = légende, mais **diagnostic erroné** sur l'impact prod L102 → corrigé ici + escaladé à Philippe pour batch 4). 0 fichier code modifié. 1 nouvelle section append-only ajoutée à SEO_PLAN.md (cette section). Branche `docs/seo-plan-statuts-actualises-t_b2d2b734` continue à porter le résidu (12e consignation, leçon #506 point 3 toujours valide).
