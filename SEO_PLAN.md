...
| 2026-09-01 | Hermes | **GSC gap T3-INFO 'como instalar tomada simples'** (t_106deaa7) | Renforcement page `client/public/blog/como-instalar-tomada-simples.html` (pos 8.6 / 69 impr / 1 clic en 28j, fenêtre 4..20 = presque top3). La page existait déjà en double dans le repo : (1) `public/blog/como-instalar-tomada-simples.html` (trackée origin/main via commit `e9254bf920` du 28/08/2026, 241 lignes, page canonique source publique) et (2) copie canonique Vite `client/public/blog/como-instalar-tomada-simples.html` (non trackée — résidu non servi en prod). Sur la copie Vite déjà en place dans le working tree (non committée), le contenu SEO était déjà renforcé : title recentré verbatim « Como Instalar Tomada Simples — Guia 7 Passos (Schuko 16A, L/N/PE) », meta keywords étoffées (22 variantes), description verbatim query, dateModified JSON-LD bump 2026-08-28 → 2026-09-01, 3 FAQ verbatim query ajoutées au schema FAQPage (Q1 « Como instalar tomada simples passo a passo? », Q2 « Como instalar tomada simples Schuko 16A? », Q3 « Como instalar uma tomada simples sozinho? »). Au cours de ce run, **deux compléments ont été ajoutés pour aligner la page avec le potentiel featured snippet + maillage cluster** : (a) **+1 H2 `id="passo-passo"`** (« Como instalar tomada simples passo a passo (resumo direto) ») avec résumé compact 7 étapes verbatim query pour cible featured snippet position 0, ajouté en TOC + section (résumé qui cite littéralement « passo a passo » — ancre SEO distincte de `id="passo"`); (b) **+2 liens internes entrants cluster** depuis `client/public/blog/como-instalar-tomada-eletrica.html` (×1 entrée « Recursos Úteis » avec ancre « 🔌 Como instalar tomada simples ») et `client/public/blog/como-instalar-tomada-usb.html` (×1 entrée « Artigos Relacionados » avec ancre « Como instalar tomada simples (passo a passo) »). **Résidu JSON-LD pré-existant** : le bloc Article de la page est corrompu par une rédaction PII automatique antérieure (`"@context":"https://***@type":"Article"` au lieu de `"@context":"https://schema.org","@type":"Article"`). Bug hérité d'une tâche précédente, présent aussi dans le source origin/main — NON traité dans ce run (scope GSC gap). À traiter dans un chantier dédié PII-rédacteurs. Aucune invention prix/zone/délai (renvoi au bloc Transparence existant alimenté par PRICING.md : 70 €/h + deslocação 15-65 € par zone + Ficha/Termo a partir de 350 €). PR draft à ouvrir depuis `origin/main` sur branche `fix/enr-rankpush-tomada-simples-r3-t_106deaa7`. Aucun merge sans GO (R7). Impact à mesurer à J+7 via gsc-trajectoire-cron.sh : cible pos < 4 (snippet potentiel + verbatim H1/H2 + FAQ enrichi + 2 liens entrants cluster + fresh dateModified). | Gap GSC fenêtre 28j (pos 8.6 = page 1 basse → top3 = levier verbatim H1/H2 + FAQ enrichi + maillage cluster + fresh dateModified) | +1 H2 / +3 FAQ verbatim / +1 title verbatim / +22 keywords / +2 inbound links cluster / +1 signal freshness / source canonique Vite créée | ⏸ PR draft en cours, attente GO Philippe (R7) |
